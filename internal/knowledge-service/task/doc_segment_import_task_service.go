...
func importDocSegment(ctx context.Context, taskCtx string) Result {
	...
	//更新状态处理中 [EN] Update status in progress
...
// doDocSegmentImport 执行文件导入 [EN] doDocSegmentImport performs file import
...
// csv 文件行处理器 [EN] csv file line processor
...
func processCsvFileLine(ctx context.Context, csvUrl string,
	...
	//下载url，循环调用rag [EN] Download url and call rag in a loop
	...
	// 创建CSV读取器 [EN] Create CSV reader
	reader := csv.NewReader(object)

	// 根据需要配置CSV读取器 [EN] Configure the CSV reader as needed
	reader.Comma = ','          // 设置分隔符，默认为逗号 [EN] Set delimiter, default is comma
	reader.Comment = '#'        // 设置注释字符 [EN] Set comment character
	reader.FieldsPerRecord = -1 // 允许可变字段数量 [EN] Allow variable number of fields

	// 读取并跳过表头行 [EN] Read and skip header rows
	_, err = reader.Read()
	if err != nil {
		if err == io.EOF {
	...
	var lineCount = 0
	// 逐行读取CSV内容 [EN] Read CSV content line by line
	for {
		...
		if err != nil {
			// 可以选择记录错误并继续，或者直接返回错误 [EN] You can choose to log the error and continue, or return the error directly
