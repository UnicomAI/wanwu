...
func (t *KnowledgeDeleteTask) Running(ctx context.Context, taskCtx string, stop <-chan struct{}) <-chan async_task.IReport {
	...
	go func() {
		...
		//执行知识库删除 [EN] Perform knowledge base deletion
...
// deleteKnowledgeByKnowledgeId 根据知识库id 删除知识库 [EN] deleteKnowledgeByKnowledgeId deletes the knowledge base based on the knowledge base id
func deleteKnowledgeByKnowledgeId(ctx context.Context, taskCtx string) Result {
	...
	//2.查询所有doc详情 [EN] 2. Query all doc details
	...
	err = db.GetClient().DB.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		...
		//删除相关权限 [EN] Delete related permissions
