...
func initKafkaAdmin() (sarama.ClusterAdmin, error) {
	...
	kafkaConfig.Net.SASL.Enable = true                                // Enable SASL authentication
	kafkaConfig.Net.SASL.User = config.GetConfig().Kafka.User         // Kafka authentication username
	kafkaConfig.Net.SASL.Password = config.GetConfig().Kafka.Password // Kafka authentication password
	kafkaConfig.Net.SASL.Handshake = true                             // Enable SASL handshake
...
func initKafka(kafkaAdmin sarama.ClusterAdmin) (sarama.SyncProducer, error) {
	...
	kafkaConfig.Version = sarama.MaxVersion
	// Use the NewBalanceStrategyRange function to set the rebalancing strategy
	...
	//kafkaConfig.Version = sarama.V2_8_1_0 //The Kafka version is selected according to the actual situation
	//kafkaConfig.Net.TLS.Enable = true // Enable TLS
	kafkaConfig.Net.SASL.Enable = true                                // Enable SASL authentication
	kafkaConfig.Net.SASL.User = config.GetConfig().Kafka.User         // Kafka authentication username
	kafkaConfig.Net.SASL.Password = config.GetConfig().Kafka.Password // Kafka authentication password
	kafkaConfig.Net.SASL.Handshake = true                             // Enable SASL handshake
	...
	// Check whether the target topic exists
	...
	if !exists {
		...
		// If the topic exists but the number of partitions is less than defaultPartitionNum, update the number of partitions
