...
// SelectKnowledgePermissionById 查询用户知识库权限 [EN] SelectKnowledgePermissionById Query user knowledge base permissions
...
// SelectKnowledgeIdByPermission 查询用户有权限的知识库id [EN] SelectKnowledgeIdByPermission queries the knowledge base id that the user has permission to
...
// SelectUserKnowledgePermissionList 查询用户知识库权限 [EN] SelectUserKnowledgePermissionList Query user knowledge base permissions
...
// SelectUserKnowledgePermission 查询用户知识库权限 [EN] SelectUserKnowledgePermission Query user knowledge base permissions
...
// CreateKnowledgeIdPermission 创建知识库权限,有可能在事务中的一部分，所以方法第一个入参为db [EN] CreateKnowledgeIdPermission creates the knowledge base permission, which may be part of the transaction, so the first parameter of the method is db
...
// BatchCreateKnowledgeIdPermission 批量创建知识库权限,有可能在事务中的一部分，所以方法第一个入参为db [EN] BatchCreateKnowledgeIdPermission batch creates knowledge base permissions, which may be part of the transaction, so the first parameter of the method is db
...
// DeleteKnowledgeIdPermission 删除知识库权限,有可能在事务中的一部分，所以方法第一个入参为db [EN] DeleteKnowledgeIdPermission deletes the knowledge base permission, which may be part of the transaction, so the first parameter of the method is db
...
// DeleteKnowledgePermissionById 删除知识库权限,有可能在事务中的一部分，所以方法第一个入参为db [EN] DeleteKnowledgePermissionById deletes the knowledge base permission, which may be part of the transaction, so the first parameter of the method is db
...
// processKnowledgeShareCount 处理知识库分享数量 [EN] processKnowledgeShareCount processes the number of knowledge base shares
...
// AsyncDeletePermissionByKnowledgeId 删除知识库权限,有可能在事务中的一部分，所以方法第一个入参为db [EN] AsyncDeletePermissionByKnowledgeId deletes the knowledge base permission, which may be part of the transaction, so the first parameter of the method is db
...
// pageDeletePermission 分页删除知识库权限 [EN] pageDeletePermission Delete knowledge base permissions by page
func pageDeletePermission(knowledgeId string) (err error) {
	var limit = 30
	var lastPermissionId = "000"
	//最多处理30w数据，理论上一个书知识库关系应该少于30w [EN] It can process up to 300,000 data. In theory, a book knowledge base relationship should be less than 300,000.
	for i := 0; i < 10000; i++ {
		var permissionList []*model.KnowledgePermission
...
// recordKnowledgePermission 记录知识库权限操作，外层保证事务 [EN] recordKnowledgePermission records knowledge base permission operations, and the outer layer guarantees transactions
...
// batchRecordKnowledgePermission 记录知识库权限操作，外层保证事务 [EN] batchRecordKnowledgePermission records knowledge base permission operations, and the outer layer guarantees transactions
...
// batchDeleteRecordKnowledgePermission 记录知识库权限操作，外层保证事务 [EN] batchDeleteRecordKnowledgePermission records knowledge base permission operations, and the outer layer guarantees transactions
