// Package assistant 是 app-service 访问 assistant-service 的 grpc 客户端封装。
//
// 当前仅用于启动时一次性数据清洗：把 app-service 各表中 app_type=agent 的
// app_id（assistant 自增老 id）批量换为 uuid。抽成接口是为了 orm 层迁移函数可 mock、不必真起 assistant。
package assistant
