...
func DataPerm(userId, orgId string) SQLOption {
	return funcSQLOption(func(db *gorm.DB) *gorm.DB {
		if userId != "" && orgId == "" {
			//Data permissions: For all individuals in the org, valid values ​​are passed for userId, and no valid values ​​are passed for orgId.
			return SQLOptions(
				WithUserId(userId),
			).Apply(db)
		} else if userId != "" && orgId != "" {
			//Data permissions: The person in this org, userId and orgId all need to pass valid values.
			return SQLOptions(
				WithUserId(userId),
				WithOrgID(orgId),
			).Apply(db)
		} else if userId == "" && orgId != "" {
			//Data permissions: All in this org, userId does not pass a valid value, orgId passes a valid value
			return SQLOptions(
				WithOrgID(orgId),
			).Apply(db)
		} else {
