...
func registerKnowledge(apiV1 *gin.RouterGroup) {
	// 知识库增删改查 [EN] Knowledge base addition, deletion, modification and query
	...
	// 知识库命中测试，通用校验不好做改内部校验 [EN] Knowledge base hit test, general verification is not easy to change internal verification
	mid.Sub("knowledge").Reg(apiV1, "/knowledge/hit", http.MethodPost, v1.KnowledgeHit, "知识库命中测试")

	// 知识库文档 [EN] Knowledge base documentation
	...
	// 知识库文档，以下通用校验不好做改内部校验 [EN] Knowledge base document, the following general verification is not easy to change the internal verification
	...
	// 知识库元数据,前端增加knowledgeId [EN] Knowledge base metadata, add knowledgeId to the front end
	...
	// 知识库文档切片增删改查 [EN] Knowledge base document slice addition, deletion, modification and query
	...
	// 知识库url文档导入,这个接口无需校验 [EN] Knowledge base URL document import, this interface does not require verification
	mid.Sub("knowledge").Reg(apiV1, "/knowledge/doc/url/analysis", http.MethodPost, v1.AnalysisDocUrl, "解析url")

	// 知识库标签增删改查 [EN] Knowledge base tag addition, deletion, modification and query
	...
	// 绑定知识库标签 [EN] Bind knowledge base tags
	...
	// 知识库关键词管理--底层进行了权限knowledgeId 过滤，此处无需处理 [EN] Knowledge base keyword management--the bottom layer has been filtered by knowledgeId, so there is no need to process it here.
	...
	// 知识库分隔符增删改查,和用户有关，和知识库无关，无需校验权限 [EN] Knowledge base separator addition, deletion, modification, and query are related to the user and have nothing to do with the knowledge base. No verification of permissions is required.
	...
	// 知识库权限 [EN] Knowledge base permissions
	...
	// 知识库社区报告 [EN] Knowledge Base Community Report
	...
	// 知识图谱详情 [EN] Knowledge graph details
