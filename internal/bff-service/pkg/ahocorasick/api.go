...
type DictStatus struct {
	DictCfg DictConfig
	Status  bool // 词表是否存在 [EN] Does the vocabulary exist?
}
...
func (a *acMgr) contentMatch(content string, dicts []DictConfig, returnFirstMatch bool) ([]MatchResult, error) {
	...
	// 1. 加读锁，仅保护字典读取阶段 [EN] 1. Add a read lock to protect only the dictionary reading phase
	a.mu.RLock()
	dictsCopy := make(map[string]*acDict) // 复制字典，避免后续操作依赖锁 [EN] Copy the dictionary to avoid subsequent operations relying on locks
	for _, cfg := range dicts {
		dictKey := getDictKey(cfg.DictID, cfg.Version)
	...
	a.mu.RUnlock() // 字典读取完毕，立即释放锁 [EN] After reading the dictionary, release the lock immediately
	...
	// 2. 无锁状态下进行匹配 [EN] 2. Matching in lock-free state
...
func (a *acMgr) contentContain(content string, dicts []DictConfig) (*DictConfig, error) {
	...
	// 2. 无锁状态匹配字段 [EN] 2. Lock-free status matching field
	contentBytes := []byte(content)
...
// getDictKey 生成字典的唯一键 [EN] getDictKey generates the unique key of the dictionary
