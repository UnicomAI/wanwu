...
func OpenAPIWorkflowRun(ctx *gin.Context, workflowID string, input []byte) ([]byte, error) {
	// 生成调用工作流的url [EN] Generate the url for calling the workflow
	// 将用户输入的intput透传 [EN] Transparently transmit the input entered by the user
...
func OpenAPIWorkflowFileUpload(ctx *gin.Context) (string, error) {
	// 从context中获取file [EN] Get file from context
	...
	// 生成文件在tos上的storeUri [EN] Generate the storeUri of the file on tos
	...
	// 使用storeUri+fileBytes上传文件 [EN] Use storeUri+fileBytes to upload files
	...
	// 生成签名，并返回可访问文件的url [EN] Generate a signature and return the url of the accessible file
