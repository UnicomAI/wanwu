...
// ListWorkflow userID/orgID data isolation, used for [workflow]
...
// ListWorkflowByIDs has no userID or orgID isolation and is used in the [Intelligent Body Selection Workflow] [Application Square] business process
func ListWorkflowByIDs(ctx *gin.Context, name string, workflowIDs []string) (*response.CozeWorkflowListData, error) {
	...
	for _, workflowID := range workflowIDs {
		if _, err := util.I64(workflowID); err == nil {
			// AgentScope Workflow ID is uuid, filter out this part of dirty data; Coze Workflow ID can be converted to numbers
...
type workflowImportData struct {
	...
	Schema string `json:"schema"` // Store as JSON string
}
