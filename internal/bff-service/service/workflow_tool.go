...
func openapiOperation2ToolActionOutputs4Workflow(operation *openapi3.Operation) []interface{} {
	...
	// 优先查找 200 响应 [EN] Find 200 responses first
	...
	// 如果没有2开头的响应，使用第一个可用的响应 [EN] If there is no response starting with 2, the first available response is used
	if responseRef == nil {
...
// openapiParameterType4Workflow 获取参数类型 [EN] openapiParameterType4Workflow gets parameter type
...
// openaiSchemaType4Workflow 获取 schema 的类型 [EN] openaiSchemaType4Workflow gets the type of schema
