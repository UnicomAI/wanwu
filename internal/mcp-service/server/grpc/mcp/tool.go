...
func (s *Service) GetToolSelect(ctx context.Context, req *mcp_service.GetToolSelectReq) (*mcp_service.GetToolListResp, error) {
	...
	// First, find out all the configured apikey built-in tools for this user, construct it into map<ToolSquareId, *model.CustomTool>, and then query the apikey through ToolSquareId
