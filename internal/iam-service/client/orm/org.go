...
func (c *Client) GetOrgAndSubOrgSelectByUser(ctx context.Context, userID, orgID uint32) ([]IDName, *errs.Status) {
	...
	return result, c.transaction(ctx, func(tx *gorm.DB) *errs.Status {
		// 获取组织树 [EN] Get organization tree
		orgTree, err := getOrgTree(tx)
		if err != nil {
			return toErrStatus("iam_orgs_select", err.Error())
		}
...
func createOrgTx(tx *gorm.DB, org *model.Org) *errs.Status {
	...
	// check parents
	if org.ParentID != 0 {
		// 正常创建组织 [EN] Create an organization normally
		...
		// check name 组织名在上级组织的所有下级组织内唯一 [EN] check name The organization name is unique within all subordinate organizations of the parent organization
		...
		// 创建系统内唯一顶级组织，此时系统内不能存在任何组织 [EN] Create the only top-level organization in the system. At this time, no organization can exist in the system.
