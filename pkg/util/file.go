...
// ToFileSizeStr fileSize单位是B，转换规则：小于1M为KB，大于等于1M，单位为M，保留两位小数 [EN] ToFileSizeStr fileSize unit is B, conversion rules: less than 1M is KB, greater than or equal to 1M, the unit is M, keep two decimal places
...
func DirFileList(dir string, subDir bool, fullPath bool) ([]string, error) {
	...
	// 读取目录 [EN] Read directory
	entries, err := os.ReadDir(dir)
	if err != nil {
		return nil, fmt.Errorf("read dir (%v) err: %v", dir, err)
	}

	// 遍历目录下的所有文件和子目录 [EN] Traverse all files and subdirectories in a directory
	for _, entry := range entries {
		info, err := entry.Info()
		if err != nil {
			// 处理错误 [EN] handling errors
			log.Errorf("read dir (%v) entry err: %v", dir, err)
			continue
		}

		// 判断是否是文件 [EN] Determine whether it is a file
		if !info.IsDir() {
			if fullPath {
		...
		} else if !subDir { //不需要校验底层目录 [EN] No need to verify the underlying directory
			continue
		} else {
...
// MergeFile 合并文件 [EN] MergeFile merge files
func MergeFile(filePathList []string, mergeFilePath string) (*FileMergeResult, error) {
	// 创建或打开文件 [EN] Create or open a file
	//0644，表示文件所有者可读写，同组用户及其他用户只可读 [EN] 0644, indicating that the file owner can read and write, and users in the same group and other users can only read.
...
func appendFile(reader *bufio.Reader, destinationFile *os.File) (byteCount int64, error error) {
	...
	for {
		...
		if FileEOF(err) { // 检查是否到达文件末尾 [EN] Check if end of file is reached
			break
		}
		if err != nil {
