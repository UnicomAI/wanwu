...
// WeekStartTime The start time of a certain week (0 o'clock on Monday), -1 last week, 0 this week, 1 next week
func WeekStartTime(t time.Time, week int) time.Time {
	offset := time.Monday - t.Weekday()
	if offset > 0 { // Special treatment on Sunday
		offset = -6
	}
	y, m, d := t.Date()
...
func Time2Date(ts int64) string {
	return time.UnixMilli(ts).In(UTC8).Format(dateFormat) // Output example: 2025-05-09
}
...
// DateRange returns [startTs, endTs] closed interval date list
...
// Returns a list of dates in the closed range of the previous period and the current period
func PreviousDateRange(startDate, endDate string) ([]string, []string, error) {
	...
	// 2. Calculate the timestamp interval of the previous period (there is no overlap in the dates of the previous and subsequent intervals)
	deltaDura := endAt - startAt + 24*time.Hour.Milliseconds()
	pervStartTs := startAt - deltaDura
	pervEndTs := endAt - deltaDura
	// 3. Calculate the previous period and current period
