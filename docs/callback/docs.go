    ...
    "paths": {
        ...
        "/app/record": {
            "post": {
                ...
                "parameters": [
                    {
                        "description": "应用使用记录请求参数",
                        "name": "data",
                        "in": "body",
                        "required": true,
                        "schema": {
                            "$ref": "#/definitions/request.AppRecordRequest"
                        }
                    }
                ],
                "responses": {
        ...
        "/digital-employee/publish/sync": {
            "post": {
                "description": "外部系统（ontology/vega）发布数字员工时回调本接口，登记 app 表 appType=digitalemployee 行（upsert，幂等）。字段对齐《数字员工与万悟交互接口契约》2.1：dh_id/name/version_id/version_no/publish_scope；身份走 Header 透传（X-User-Id/X-Org-Id，缺省回退 x-account-id）。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
            ...
            "delete": {
                "description": "外部系统（ontology/vega）删除数字员工时回调本接口，删除 app 表 appType=digitalemployee 行（以 dh_id 为准，幂等，目标不存在也返回成功）。字段对齐《数字员工与万悟交互接口契约》2.2：dh_id/version_no；身份走 Header 透传。",
                "consumes": [
                    "application/json"
                ],
                "produces": [
    ...
    "definitions": {
        ...
        "request.DeleteDigitalEmployeePublishSyncReq": {
            ...
            "properties": {
                ...
                "version_no": {
                    "description": "删除时当前发布版本号（仅辅助校验/审计，当前不落库）",
                    "type": "string"
                }
            }
        },
        "request.DigitalEmployeePublishSyncReq": {
            ...
            "properties": {
                ...
                "name": {
                    "description": "数字员工名称（契约字段，当前 app 表不落库）",
                    "type": "string"
                },
                "publish_scope": {
                    "description": "发布范围(public/organization/private)",
                    "type": "string"
                },
                "version_id": {
                    "description": "版本记录ID（契约字段，当前不落库）",
                    "type": "string"
                },
                "version_no": {
                    "description": "发布版本号（契约字段，当前不落库）",
                    "type": "string"
                }
            }
        },
