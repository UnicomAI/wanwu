...
| file_info | 否       | array  | 随问题携带的文件，元素含 fileName、fileUrl、fileSize。fileUrl 填[上传文件API](../../知识库API/2.文档管理/1.上传文件API.md)返回的 filePath。**当前仅支持 1 张图片**（png/jpg/jpeg），且需打开视觉开关、对话模型支持图文问答、绑定多模态知识库、知识库 Rerank 为多模态 Rerank，详见[文件上传API](../4.文件管理/1.文件上传API.md) |
