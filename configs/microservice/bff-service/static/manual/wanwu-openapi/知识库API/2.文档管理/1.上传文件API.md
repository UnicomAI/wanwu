...
多模态知识库还支持：视频avi/mp4/mov/wmv（每个不超过100MB）、图片png/jpg/jpeg（每个不超过3MB）、音频mp3/wav/aac/m4a（大小限制由所选ASR模型决定）。
...
> **要导入知识库的文件不要传 `isExpired=1`**：[文档导入API](2.文档导入API.md) 只在过期目录下按 `fileId` 找文件，传了 `1` 会导入失败并报 `142007 获取上传文档失败，请稍后重试`。用默认（过期）上传即可。
