...
| Parameter  | Type     | Instructions   |
| ---- | ------ | ---- |
| prompt     | string | 拼接了命中内容的提示词 |
| searchList | array  | 命中的知识片段列表 |
| score      | float array | 各片段的相似度打分 |
| useGraph   | bool   | 是否使用了知识图谱 |
...
| childContentList | array  | 子分段内容列表（父子分段时返回，元素含 `childSnippet`/`score`） |
| childScore       | float array | 子分段打分 |
| contentType      | string | 内容类型：text-文档分段，graph-知识图谱，community_report-社区报告，qa-问答库 |
| score            | float  | 片段相似度打分 |
| rerankInfo       | array  | 重排序信息（元素含 `type`/`fileUrl`/`score`） |
| metaData         | object | 命中片段元信息，对象内容原样透传知识库检索结果，常见字段见下表 |
...
> metaData 为透传字段，实际字段以检索结果为准，可能随版本增减，调用方请按可选字段处理。
    ...
    "searchList": [
      {
        ...
        "metaData": {
          "file_name": "AI科技中心开票信息.docx",
          "download_link": "http://localhost:8081/minio/download/api/rag-doc/59bfaef5-8609-48ad-a69d-305fcd221e93.docx?X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Expires=86400&X-Amz-Signature=xxx",
          "page_num": [1],
          "parent_title": [],
          "row_num": 0,
          "chunk_current_num": 1,
          "chunk_total_num": 1,
          "bucket_name": "rag-doc",
          "object_name": "59bfaef5-8609-48ad-a69d-305fcd221e93.docx",
          "doc_meta": []
        }
      }
    ],
