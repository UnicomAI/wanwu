    ...
    <div class="page-wrapper-content">
      ...
      <div class="page-container">
        ...
        <div class="page-container-right">
          <div class="group-header">
            <h2>
              {{ $t('adminCenter.pageModules.resourcePool.tool.title') }}
            </h2>
          </div>
          <admin-search-bar
          ...
          >
            <template #extra>
              ...
              >
                ...
                <el-input
                  v-model="toolSearchForm.name"
                  class="admin-search-bar__control"
                  clearable
                  prefix-icon="el-icon-search"
                  :placeholder="$t('adminCenter.placeholders.toolName')"
              ...
              <!-- <div class="admin-search-bar__field">
                ...
                <el-select
                  v-model="toolSearchForm.publishStatus"
                  class="admin-search-bar__control"
                  multiple
                  collapse-tags
                  @change="handleToolMultiSelectChange('publishStatus', $event)"
                >
                  <el-option
...
import { getAdminToolPageList } from '@/api/adminCenter';
import {
  ALL_VALUE,
  PUBLISH_STATUS_LIST,
...
export default {
  ...
  data() {
    return {
      listStateType: 'tool',
      listStateSearchFormKey: 'toolSearchForm',
      listStatePreviousSelectionsKey: 'previousToolSelections',
  ...
  methods: {
    ...
    createListParams(commonForm = this.commonSearchForm) {
      ...
      const params = normalizeAdminListParams({
        ...
        ...commonForm,
        name: this.toolSearchForm.name,
        publishStatus: this.toolSearchForm.publishStatus,
        orgIdList,
        isAllOrg,
      });
      return params;
    },
    handleToolMultiSelectChange(field, values) {
      const previous = this.previousToolSelections[field] || [ALL_VALUE];
      ...
      this.toolSearchForm[field] = nextValues;
      this.previousToolSelections[field] = [...nextValues];
      this.tablePage = 1;
      this.fetchTableData();
    },
    async requestTableData(commonForm = this.commonSearchForm, requestId) {
      ...
      try {
        const res = await getAdminToolPageList(
    ...
    handleViewDetail(row) {
      this.saveListState();
      this.$router.push({
        path: `/adminCenter/tool/detail?toolId=${row.toolId || row.id}`,
      });
    },
  },
  mounted() {
