    ...
    <div class="page-wrapper-content">
      ...
      <div class="page-container">
        ...
        <div class="page-container-right">
          <div class="group-header">
            <h2>
              {{ $t('adminCenter.pageModules.appDevelopment.workflow.title') }}
            </h2>
          </div>
          <admin-search-bar
          ...
          >
            <template #extra>
              ...
              >
                ...
                <el-input
                  v-model="workflowSearchForm.name"
                  class="admin-search-bar__control"
                  clearable
                  prefix-icon="el-icon-search"
                  :placeholder="$t('adminCenter.placeholders.workflowName')"
                  @keyup.enter.native="handleSearch(commonSearchForm)"
                />
              </div>
              <div class="admin-search-bar__field">
                <div class="admin-search-bar__label">
                  {{ $t('appSpace.selectWorkflowType') }}
                </div>
                <el-select
                  v-model="workflowSearchForm.appType"
                  class="admin-search-bar__control"
                  multiple
                  collapse-tags
                  @change="handleWorkflowMultiSelectChange('appType', $event)"
                >
                  <el-option
              ...
              <div class="admin-search-bar__field">
                ...
                <el-select
                  v-model="workflowSearchForm.publishStatus"
                  class="admin-search-bar__control"
                  multiple
                  collapse-tags
                  @change="
                    handleWorkflowMultiSelectChange('publishStatus', $event)
                  "
                >
                  <el-option
...
import { getAdminWorkflowPageList } from '@/api/adminCenter';
import {
  ALL_VALUE,
  PUBLISH_STATUS_LIST,
...
export default {
  ...
  data() {
    return {
      listStateType: 'workflow',
      listStateSearchFormKey: 'workflowSearchForm',
      listStatePreviousSelectionsKey: 'previousWorkflowSelections',
  ...
  computed: {
    workflowTypeOptions() {
      return [
        ...
        {
          label: this.$t('appSpace.workflow'),
          value: 'workflow',
        },
        {
          label: this.$t('appSpace.chat'),
          value: 'chatflow',
        },
      ];
    },
    ...
    tableColumns() {
      return [
        ...
        {
          prop: 'appType',
          label: this.$t('appSpace.selectWorkflowType'),
          type: 'ellipsis',
          minWidth: 100,
          formatter: row => this.getWorkflowTypeName(row.appType),
        },
        {
          prop: 'ownerUserName',
  ...
  methods: {
    ...
    createListParams(commonForm = this.commonSearchForm) {
      ...
      const params = normalizeAdminListParams({
        ...
        ...commonForm,
        name: this.workflowSearchForm.name,
        appType: this.workflowSearchForm.appType,
        publishStatus: this.workflowSearchForm.publishStatus,
        orgIdList,
        isAllOrg,
      });
      return params;
    },
    handleWorkflowMultiSelectChange(field, values) {
      const previous = this.previousWorkflowSelections[field] || [ALL_VALUE];
      ...
      this.workflowSearchForm[field] = nextValues;
      this.previousWorkflowSelections[field] = [...nextValues];
      this.tablePage = 1;
      this.fetchTableData();
    },
    async requestTableData(commonForm = this.commonSearchForm, requestId) {
      ...
      try {
        const res = await getAdminWorkflowPageList(
    ...
    handleReset(commonForm) {
      ...
      this.workflowSearchForm = {
        name: '',
        appType: [ALL_VALUE],
        publishStatus: [ALL_VALUE],
      };
      this.previousWorkflowSelections = {
        appType: [ALL_VALUE],
        publishStatus: [ALL_VALUE],
      };
      this.tablePage = 1;
