    ...
    <div class="page-wrapper-content">
      ...
      <div class="page-container">
        ...
        <div class="page-container-right">
          <div class="group-header">
            <h2>
              {{ $t('adminCenter.pageModules.modelService.modelManage') }}
            </h2>
          </div>
          <admin-search-bar
          ...
          >
            <template #extra>
              <!-- <div class="admin-search-bar__field">
                ...
                <el-select
                  v-model="modelSearchForm.publishStatus"
                  class="admin-search-bar__control"
                  multiple
                  collapse-tags
                  @change="
                    handleModelMultiSelectChange('publishStatus', $event)
                  "
                >
                  <el-option
              ...
              >
                ...
                <el-input
                  v-model="modelSearchForm.name"
                  class="admin-search-bar__control"
                  clearable
                  prefix-icon="el-icon-search"
                  :placeholder="$t('adminCenter.placeholders.modelName')"
                  @keyup.enter.native="handleSearch(commonSearchForm)"
                />
              </div>
              <div class="admin-search-bar__field basis-220">
                <div class="admin-search-bar__label">
                  {{ $t('adminCenter.columns.modelType') }}
                </div>
                <el-select
                  v-model="modelSearchForm.modelType"
                  class="admin-search-bar__control"
                  multiple
                  collapse-tags
                  @change="handleModelMultiSelectChange('modelType', $event)"
                >
                  <el-option
...
export default {
  ...
  computed: {
    ...
    modelTypeOptions() {
      return [
        {
          key: ALL_VALUE,
          name: this.$t('adminCenter.options.common.all'),
        },
        ...MODEL_TYPE,
      ];
    },
      ...
      return [
        {
          ...
          minWidth: 150,
          formatter: row => row.name || row.displayName || row.model,
        },
        {
          prop: 'modelDesc',
          ...
          type: 'ellipsis',
          minWidth: 150,
          formatter: row => row.modelDesc || row.desc,
        },
        {
          prop: 'modelType',
          label: this.$t('adminCenter.columns.modelType'),
          type: 'ellipsis',
          minWidth: 120,
          formatter: row => this.getModelTypeName(row.modelType || row.type),
        },
        {
          prop: 'provider',
          label: this.$t('adminCenter.columns.provider'),
          type: 'ellipsis',
          minWidth: 120,
          formatter: row => PROVIDER_OBJ[row.provider] || row.provider,
        },
        {
          prop: 'ownerUserName',
  ...
  methods: {
    getModelTypeName(type) {
      const modelType = this.modelTypeOptions.find(item => item.key === type);
      return modelType ? modelType.name : type;
    },
    ...
    handleModelMultiSelectChange(field, values) {
      const previous = this.previousModelSelections[field] || [ALL_VALUE];
