    ...
    <div class="page-wrapper-content">
      ...
      <div class="page-container">
        ...
        <div class="page-container-right">
          <div class="group-header">
            <h2>
              {{ $t('adminCenter.pageModules.appDevelopment.agent.title') }}
            </h2>
          </div>
          <admin-search-bar
          ...
          >
            <template #extra>
              ...
              >
                ...
                <el-input
                  v-model="agentSearchForm.name"
                  class="admin-search-bar__control"
                  clearable
                  prefix-icon="el-icon-search"
                  :placeholder="$t('adminCenter.placeholders.agentName')"
                  @keyup.enter.native="handleSearch(commonSearchForm)"
                />
              </div>
              <div class="admin-search-bar__field basis-220">
                <div class="admin-search-bar__label">
                  {{ $t('adminCenter.columns.agentType') }}
                </div>
                <el-select
                  v-model="agentSearchForm.category"
                  class="admin-search-bar__control"
                  multiple
                  collapse-tags
                  @change="handleAgentMultiSelectChange('category', $event)"
                >
                  <el-option
              ...
              <div class="admin-search-bar__field">
                ...
                <el-select
                  v-model="agentSearchForm.publishStatus"
                  class="admin-search-bar__control"
                  multiple
                  collapse-tags
                  @change="
                    handleAgentMultiSelectChange('publishStatus', $event)
                  "
                >
                  <el-option
...
import { getAdminAssistantPageList } from '@/api/adminCenter';
import {
  ALL_VALUE,
  PUBLISH_STATUS_LIST,
...
export default {
  ...
  data() {
    return {
      listStateType: 'agent',
      listStateSearchFormKey: 'agentSearchForm',
      listStatePreviousSelectionsKey: 'previousAgentSelections',
  ...
  computed: {
    ...
    agentTypeOptions() {
      return [
        ...
        {
          label: this.$t('adminCenter.options.agentType.single'),
          value: 1,
        },
        {
          label: this.$t('adminCenter.options.agentType.multiple'),
          value: 2,
        },
      ];
    },
    tableColumns() {
      return [
        ...
        {
          prop: 'category',
          label: this.$t('adminCenter.columns.agentType'),
          type: 'ellipsis',
          minWidth: 110,
          formatter: row => {
            const category = Number(row.category);
            if (category === 1) {
              return this.$t('adminCenter.options.agentType.single');
            }
            if (category === 2) {
              return this.$t('adminCenter.options.agentType.multiple');
            }
            return '-';
          },
        },
  ...
  methods: {
    createListParams(commonForm = this.commonSearchForm) {
      ...
      const params = normalizeAdminListParams({
        ...
        ...commonForm,
        name: this.agentSearchForm.name,
        category: this.agentSearchForm.category,
        publishStatus: this.agentSearchForm.publishStatus,
        orgIdList,
        isAllOrg,
      });
      return params;
    },
    handleAgentMultiSelectChange(field, values) {
      const previous = this.previousAgentSelections[field] || [ALL_VALUE];
      ...
      this.agentSearchForm[field] = nextValues;
      this.previousAgentSelections[field] = [...nextValues];
      this.tablePage = 1;
      this.fetchTableData();
    },

    async requestTableData(commonForm, requestId) {
      this.tableLoading = true;
      try {
        const res = await getAdminAssistantPageList(
    ...
    handleViewDetail(row) {
      this.saveListState();
      this.$router.push({
        path: `/adminCenter/agent/detail?assistantId=${row.appId || row.id}`,
      });
    },
  },
