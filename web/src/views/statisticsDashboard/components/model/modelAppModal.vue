...
export default {
  components: { Pagination, ModelBasicInfo, ModelTypeTag, AppTypeTag },
  props: {
    visible: {
      type: Boolean,
      default: false,
  ...
  methods: {
    ...
    async exportData() {
      const response = await exportModelAppData(this.modalAppParams);
      resDownloadFile(
        response,
        `${this.$t('statisticsDashboard.modelStatistics')}_${this.$t('statisticsDashboard.appUsageStats')}.xlsx`,
      );
    },
  },
};
