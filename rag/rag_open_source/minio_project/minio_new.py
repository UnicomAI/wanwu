...
# ======= 从环境变量中获取mimio配置信息 ======= [EN] ======= Obtain mimio configuration information from environment variables =======
...
# ======= 从环境变量中获取mimio配置信息 ======= [EN] ======= Obtain mimio configuration information from environment variables =======
# 配置Minio服务器 [EN] Configure Minio server
minio_client = Minio(
    MINIO_ADDRESS,
...
def upload_file_to_minio(file_stream, original_filename, bucket_name=default_bucket_name, overwrite_file_name=None):
    try:
        # 获取文件扩展名 [EN] Get file extension
        _, file_extension = os.path.splitext(original_filename)
        
        # 创建临时文件 [EN] Create temporary files
        temp_file_path = tempfile.mktemp(suffix=file_extension)
        with open(temp_file_path, "wb") as temp_file:
            temp_file.write(file_stream.read())

        # 获取临时文件的文件名 [EN] Get the filename of a temporary file
...
def upload_file():
    try:
        # 从请求中获取文件 [EN] Get files from request
