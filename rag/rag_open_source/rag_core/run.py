...
@app.route('/rag/init-knowledge-base', methods=['POST'])  # initialization done
def init_kb():
    logger.info('---------------初始化知识库---------------')
    try:
...
# # ************************* Synchronous upload API interface, close and not in use ****************************

@app.route("/rag/add-knowledge-temp", methods=["POST", "GET"])  # Add a single file
...
# @app.route("/rag/batch-add-knowledge", methods=["POST","GET"]) #Add multiple files
# def batch_add_konwledge():
#     logger.info('---------------Batch upload files---------------')
#     response_info = {
#         'code': 0,
#         "message": "success"
#     }
...
#         if files is None:
#             response_info["code"] = 1
#             response_info["message"] = "File upload failed"
#             json_str = json.dumps(response_info, ensure_ascii=False)
#             response = make_response(json_str)
#             response.headers['Access-Control-Allow-Origin'] = '*'
#             return response
...
@app.route("/rag/del-knowledge-base", methods=["POST"])  # Delete knowledge base done
def del_kb():
    logger.info('---------------删除知识库---------------')
    try:
        init_info = json.loads(request.get_data())
        ...
        # Add deletion reids logic to the batch deletion file begin
        ...
        # Add deletion reids logic to the batch deletion file end
        # ========== Chunk labels deletion logic ==========
        try:
            if not kb_id:
                kb_id = kb_utils.get_kb_name_id(user_id, kb_name)  # Get kb_id
            # Delete chunk_labels
...
@app.route("/rag/search-knowledge-base", methods=["POST"])  # Query done
def search_knowledge_base():
    ...
    try:
        ...
        # The rerank model needs to be specified when answering questions in the open source version of Dify
...
@app.route("/rag/list-knowledge-base", methods=["POST"])  # Query all knowledge base names under the user done
def list_kb():
    logger.info('---------------查询所有知识库---------------')
    try:
        init_info = json.loads(request.get_data())
...
@app.route("/rag/list-knowledge-file", methods=["POST"])  # Show all files under user knowledge base done
def list_file():
...
@app.route("/rag/list-knowledge-file-download-link", methods=["POST"])  # Show all files under user knowledge base done
...
@app.route("/rag/del-knowledge-file", methods=["POST"])  # Delete a single file under a knowledge base done
def del_file():
    logger.info('---------------删除知识库文件---------------')
    try:
        init_info = json.loads(request.get_data())
...
@app.route("/rag/batch_del_knowfiles", methods=["POST"])  # Delete multiple files under a knowledge base done
def del_files():
...
@app.route("/rag/check-knowledge-base", methods=["POST"])  # Query whether a certain knowledge base is under a certain user done
def check_kb():
...
def getContentList():
    ...
    try:
        ...
        # Get a list of paginated file contents
...
def updateContentStatus():
    ...
    try:
        ...
        status = data['status']
        on_off_switch = data.get('on_off_switch', None)  # Defaults to None if not passed
...
def truncate_filename(filename, max_length=200):
    ...
    # Trim 255 characters from back to front, making sure to keep the extension
...
def doc_parser():
    ...
    try:
        ...
        # Truncate the file name: When the file name is too long and exceeds the maximum length allowed by the system, please truncate 200 characters from the back to the front.
...
def user_feedback():
    ...
    try:
        ...
        action = init_info.get("action", "")  # like: like; stomp: click; cancel: cancel
        answer = init_info.get("answer", "") # Answer
        error_type = init_info.get("error_type", "") #all_error: all errors; part_error: partial errors; other: other
        other_reason = init_info.get("other_reason", "") # Other reasons
        source = init_info.get("source", "") # Call source: ChatConsult or Agent. If the value is empty, it may be an API call.
...
def proper_noun():
    ...
    try:
        ...
        action = init_info.get("action", "")  # add: add; delete: delete; update: modify
        name = init_info.get("name", "")  # proper noun
        alias = init_info.get("alias", [])  # alias list
        # apply_type = init_info.get("apply_type", []) # Scope: user or knowledgebase or user + knowledgebase
        # knowledge_base = init_info.get("knowledge_base_list", []) # If the scope is knowledgebase, a list of knowledge base names needs to be passed
        knowledge_base_info = init_info.get("knowledge_base_info", {})
        ...
        if msg_id and action and knowledge_base_info:  # Note that knowledge_base contains kb_ids
