...
class DOCXLoader(TextLoader):
    ...
    def custom_load_and_split_doc(self) -> (List[dict]):
        ...
        text_splitter = CharacterTextSplitter(
            ...
            length_function=len_fun,  # Calculate length in tokens
        ...
        try:
            ...
            chunk_order = 0
            for doc in docs:  # Added to chunks and sub_chunks
                embedding_chunks = []
                chunk_order += 1
...
def docx_to_markdown(docx_path):
    ...
    for element in doc.element.body.iterchildren():
        if isinstance(element, docx.oxml.text.paragraph.CT_P):
            ...
            if '<w:drawing>' in element.xml:  # Determine whether it is an illustration
                # regular expression
                ...
                if match:
                    try:
                        image_rid = match.group(1)  # Get the contents of a capturing group
            ...
            # Processing forms
            markdown_text += '\n'  # Add a blank row before the table
    ...
    else:  # If the first method does not extract text, use the second method
...
if __name__ == "__main__":
    ...
    # ============ The test uses docx to read the document and convert it to markdown format =============
