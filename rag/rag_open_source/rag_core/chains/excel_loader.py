...
##
# def trans_excel(filepath,sheet_name="故障清单"): [EN] def trans_excel(filepath,sheet_name="Fault List"):
#     data = pd.read_excel(filepath,sheet_name=sheet_name)
#     with open(filepath.replace(".xlsx", ".txt"),"w") as f:
#         for i,d in enumerate(data.values):
#             text = str(d[2]) + "的" + str(d[3]) + "的"+ str(d[4]) + "的"+ str(d[5]) + "的"+ str(d[7]) + "的零部件" +str(d[8])+"出现"+str(d[9])+"。故障现象为" + str(d[14]) + "。根本原因为" +str(d[15])+"。它的解决方案为"+str(d[16]) [EN] text = str(d[2]) + "of" + str(d[3]) + "of"+ str(d[4]) + "of"+ str(d[5]) + "of"+ str(d[7]) + "of parts" +str(d[8])+"appears"+str(d[9])+". The fault phenomenon is "+ str(d[14]) + ". The root cause is" +str(d[15])+". Its solution is "+str(d[16])
#             f.write(text+"\n")
    ...
    def load(self) -> List[Document]:
        ...
        try:
            ...
            for sheet_name in valid_sheet_list:
                ws = wb[sheet_name]
                # ws = wb.active
                # 打印总行数 [EN] Print total number of lines
                total_rows = int(ws.max_row)

                logger.info(f"=======>Total rows in '{file_name}_{sheet_name}': {total_rows}")
                # 获取所有合并单元格的范围 [EN] Get the range of all merged cells
                merged_cells_ranges = ws.merged_cells.ranges
                # 将合并单元格范围转换为更易于处理的格式 [EN] Convert merged cell ranges to a more manageable format
                merged_ranges = []
                ...
                for row_idx, row in enumerate(ws.iter_rows(min_row=2, max_col=len(columns), values_only=True), start=2):
                    # 检查行是否全为空 [EN] Check if the rows are all empty
                    if all(cell is None for cell in row):
                        continue  # 跳过空行 [EN] Skip empty lines


                    for idx, cell_value in enumerate(row, start=1):

                        # 检查单元格是否在合并单元格范围内 [EN] Check if cell is within merged cell range
                        column_letter = get_column_letter(idx)  # 直接使用列索引获取列字母 [EN] Get column letters directly using column index
                        # 检查单元格是否在合并单元格范围内 [EN] Check if cell is within merged cell range
                        cell_position = f'{column_letter}{row_idx}'
                        col_num, row_num = coordinate_from_string(cell_position)
                        col_num_index = column_index_from_string(col_num)  # 将列字母转换为列号 [EN] Convert column letters to column numbers
                        ...
                        if cell_value is None and any(
                            ...
                            text = text + column_name + ":" + get_last_value + ";"
                            # 根据需求处理合并单元格的值 [EN] Process the values ​​of merged cells as needed
                            ...
                            # 若本单元格有值且存在跨单元格合并则缓存到历史dict中 [EN] If this cell has a value and there is a cross-cell merge, it will be cached in the historical dict.
