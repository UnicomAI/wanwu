...
if __name__ == "__main__":
    ...
    excel_file = "graph_schema_文物.xlsx"  # 或 "图谱schema定义模板.xls" [EN] or "Graph schema definition template.xls"

    try:
        schema_json = parse_excel_to_schema_json(excel_file)
        # 输出格式化 JSON 到控制台 [EN] Output formatted JSON to console
        print(json.dumps(schema_json, ensure_ascii=False, indent=4))

        # （可选）保存到文件 [EN] (Optional) Save to file
        # with open("schema_output.json", "w", encoding="utf-8") as f:
        #     json.dump(schema_json, f, ensure_ascii=False, indent=4)
        # print("✅ 已保存到 schema_output.json") [EN] print("✅ saved to schema_output.json")
