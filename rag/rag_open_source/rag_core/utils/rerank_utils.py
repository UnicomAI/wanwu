...
def get_maas_rerank(rerank_url, model_name, api_key, rerank_data, raw_search_list, top_k):
    ...
    while retrys_num < 1:
        try:
            ...
            sorted_scores = []
            sorted_search_list = []
            for item in result_data[:top_k]:  # 只取前top_k个结果 [EN] Only take the first top_k results
            ...
            retrys_num += 1
            time.sleep(0.5)
    return {"code": 1, "msg": f"{model_name} rerank error: {rerank_url}"}  # 返回错误信息 [EN] Return error message
...
def get_openai_rerank(rerank_url, model_name, api_key, query, documents, raw_search_list, top_n):
    ...
    # 将请求体转换为JSON字符串用于日志记录 [EN] Convert request body to JSON string for logging
    request_body = json.dumps(rerank_data, ensure_ascii=False, indent=2)
    logger.info(f"Rerank request details:\nURL: {rerank_url}\nHeaders: {headers}\nBody: {request_body}")
    
    for attempt in range(3):  # 重试3次 [EN] Retry 3 times
        try:
            # 发送请求（注意使用request_body变量会二次编码，应使用原始数据） [EN] Send a request (note that using the request_body variable will result in secondary encoding, the original data should be used)
            ...
            # 记录原始响应以便调试 [EN] Log the raw response for debugging
            ...
            # 关键检查：确保结果字段存在 [EN] Key check: Make sure the result field exists
            if "results" not in results:
                raise KeyError(f"Missing 'results' field in response: {results.keys()}")
                
            # 处理正常结果 [EN] Handle normal results
            ...
            time.sleep(0.5 * (attempt + 1))  # 指数退避 [EN] exponential backoff
...
def get_model_rerank(question, top_k, searchList, es_list, rerank_model_id, term_weight_coefficient=1):
    ...
    if model_config.model_provider == "OpenAI-API-compatible":
        ...
        result_data = get_openai_rerank(model_url, model_name, api_key, question, documents, raw_search_list,
                                        top_k)  # 不传 len(raw_search_list) [EN] Do not pass len(raw_search_list)
        ...
        raw_search_list = gen_raw_list(searchList, es_list)
        texts = [item['snippet'] for item in raw_search_list]  # 从字典中提取文本 [EN] Extract text from dictionary
    ...
    # # =========== 最后过一遍 hybrid_term_weight_rerank ========= [EN] # =========== One last pass hybrid_term_weight_rerank =========
    # rerank_search_list = sorted_search_list
    # rerank_scores = sorted_scores
    # # term_weight_coefficient = 1  # 关键词得分系数，默认 1 [EN] # term_weight_coefficient = 1 # Keyword score coefficient, default 1
...
def rerank_search(question, sorted_scores, search_list, threshold, return_meta, prompt_template, default_answer,
    ...
    try:
        ...
        # 判断是否临时截断 context [EN] Determine whether to temporarily truncate context
        if TRUNCATE_PROMPT:
...
def gen_rerank_search_list(milvus_list, es_list, search_field="content"):
    ...
    milvus_search_list = []
    es_search_list = []
    milvus_dup_content = []  # 去重 [EN] Remove duplicates
    es_dup_content = []  # 去重 [EN] Remove duplicates
...
def extract_keyword_entities(query):
    ...
    """
    # 定义正则表达式模式，用于匹配非纯字母和非纯数字的序列 [EN] Define a regular expression pattern to match sequences of non-pure letters and non-pure numbers
    # 匹配由 a-zA-Z0-9_- 组成的连续串（长度 >= 2），并保证前后不是这些字符（保证整片段） [EN] Matches a continuous string (length >= 2) consisting of a-zA-Z0-9_-, and ensures that the characters before and after are not these (the entire fragment is guaranteed)
    sequence_pattern = r'(?<![A-Za-z0-9_-])[A-Za-z0-9_-]{2,}(?![A-Za-z0-9_-])'
    raw_matches = re.findall(sequence_pattern, query)
    # 过滤掉纯字母或纯数字 [EN] Filter out pure letters or pure numbers
    sequence_entities = [m for m in raw_matches if not re.fullmatch(r'[A-Za-z]+', m) and not re.fullmatch(r'\d+', m)]
    # 使用jieba.posseg进行分词和词性标注 [EN] Use jieba.posseg for word segmentation and part-of-speech tagging
    words = jieba.lcut(query)
    word_pos_list = [(word, pseg.lcut(word)[0].flag) for word in words]

    # 保留特定词性的词，例如名词（n）、地名（ns）、专有名词（nz）、机构名（nt）、数字（m）、数量词（mq） [EN] Words that retain specific parts of speech, such as nouns (n), place names (ns), proper nouns (nz), organization names (nt), numbers (m), quantifiers (mq)
    keyword_entities = [word for word, pos in word_pos_list if pos in ['eng', 'n', 'ns', 'nz', 'nt', 'm', 'mq']]

    # 返回提取结果 [EN] Return extraction results
    return {
...
def get_keyword_tfidf_scores(keyword_entities, search_list):
    ...
    # 自定义分词函数 [EN] Custom word segmentation function
    def jieba_tokenize(text):
        return jieba.lcut(text)  # 使用精确模式分词 [EN] Use exact pattern word segmentation

    # 提取搜索结果中的文本片段 [EN] Extract text fragments from search results
    context_list = [x["snippet"] for x in search_list]

    # 初始化TfidfVectorizer，并传入自定义分词函数 [EN] Initialize TfidfVectorizer and pass in the custom word segmentation function
    vectorizer = TfidfVectorizer(tokenizer=jieba_tokenize, lowercase=False)

    # 计算TF-IDF矩阵 [EN] Calculate TF-IDF matrix
    tfidf_matrix = vectorizer.fit_transform(context_list)

    # 获取词汇表 [EN] Get glossary
...
def hybrid_term_weight_rerank(query, search_list, scores=[], top_k=5, term_weight_coefficient=1, threshold=0):
    ...
    返回:
        tuple: 包含两个元素：
            ...
            - res_score: 对应的得分列表。
    """
    if not scores:  # 若无scores，初始化一个 [EN] If there are no scores, initialize one
        scores = [0 for _ in search_list]

    # 提取关键实体和关键词 [EN] Extract key entities and keywords
    result = extract_keyword_entities(query)
    sequence_entities = result["sequence_entities"]
    keyword_entities = result["keyword_entities"]

    # 计算序列实体和关键词的得分 [EN] Calculate scores for sequence entities and keywords
    ...
    # 使用列表推导式和zip函数计算综合得分 [EN] Compute overall score using list comprehension and zip function
    ...
    # 重新按得分大小排序 [EN] Reorder by score size
    sorted_pairs = sorted(zip(hybrid_scores, search_list), key=lambda x: x[0], reverse=True)

    # 分别提取排序后的search_list和score [EN] Extract the sorted search_list and score respectively
    res_score = [min(1, pair[0]) for pair in sorted_pairs if pair[0]>=threshold]  # 得分限制在0-1之间 [EN] Score is limited to 0-1
    res_search_list = [pair[1] for pair in sorted_pairs][:len(res_score)]

    # 返回重排序结果 [EN] Return reordered results
