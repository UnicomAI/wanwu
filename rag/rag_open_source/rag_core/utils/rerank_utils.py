...
def get_maas_rerank(rerank_url, model_name, api_key, rerank_data, raw_search_list, top_k):
    ...
    while retrys_num < 1:
        try:
            ...
            sorted_scores = []
            sorted_search_list = []
            for item in result_data[:top_k]:  # Only take the first top_k results
            ...
            retrys_num += 1
            time.sleep(0.5)
    return {"code": 1, "msg": f"{model_name} rerank error: {rerank_url}"}  # Return error message
...
def get_openai_rerank(rerank_url, model_name, api_key, query, documents, raw_search_list, top_n):
    ...
    for attempt in range(3):  # Retry 3 times
        try:
            # Send a request (note that using the request_body variable will result in secondary encoding, the original data should be used)
            ...
            # Key check: Make sure the result field exists
...
def get_model_rerank(question, top_k, searchList, es_list, rerank_model_id, term_weight_coefficient=1):
    ...
    if model_config.model_provider == "OpenAI-API-compatible":
        ...
        result_data = get_openai_rerank(model_url, model_name, api_key, question, documents, raw_search_list,
                                        top_k)  # Do not pass len(raw_search_list)
        ...
        raw_search_list = gen_raw_list(searchList, es_list)
        texts = [item['snippet'] for item in raw_search_list]  # Extract text from dictionary
    ...
    # # =========== One last pass hybrid_term_weight_rerank =========
    # rerank_search_list = sorted_search_list
    # rerank_scores = sorted_scores
    # # term_weight_coefficient = 1 # Keyword score coefficient, default 1
...
def rerank_search(question, sorted_scores, search_list, threshold, return_meta, prompt_template, default_answer,
    ...
    try:
        ...
        # Determine whether to temporarily truncate context
        if TRUNCATE_PROMPT:
...
def gen_rerank_search_list(milvus_list, es_list, search_field="content"):
    ...
    milvus_search_list = []
    es_search_list = []
    milvus_dup_content = []  # Remove duplicates
    es_dup_content = []  # Remove duplicates
...
def extract_keyword_entities(query):
    ...
    """
    # Define a regular expression pattern to match sequences of non-pure letters and non-pure numbers
    # Matches a continuous string (length >= 2) consisting of a-zA-Z0-9_-, and ensures that the characters before and after are not these (the entire fragment is guaranteed)
    sequence_pattern = r'(?<![A-Za-z0-9_-])[A-Za-z0-9_-]{2,}(?![A-Za-z0-9_-])'
    raw_matches = re.findall(sequence_pattern, query)
    # Filter out pure letters or pure numbers
    sequence_entities = [m for m in raw_matches if not re.fullmatch(r'[A-Za-z]+', m) and not re.fullmatch(r'\d+', m)]
    # Use jieba.posseg for word segmentation and part-of-speech tagging
    words = jieba.lcut(query)
    word_pos_list = [(word, pseg.lcut(word)[0].flag) for word in words]

    # Words that retain specific parts of speech, such as nouns (n), place names (ns), proper nouns (nz), organization names (nt), numbers (m), quantifiers (mq)
...
def get_keyword_tfidf_scores(keyword_entities, search_list):
    ...
    # Custom word segmentation function
    def jieba_tokenize(text):
        return jieba.lcut(text)  # Use exact pattern word segmentation

    # Extract text fragments from search results
    context_list = [x["snippet"] for x in search_list]

    # Initialize TfidfVectorizer and pass in the custom word segmentation function
...
def hybrid_term_weight_rerank(query, search_list, scores=[], top_k=5, term_weight_coefficient=1, threshold=0):
    ...
    返回:
        tuple: 包含两个元素：
            ...
            - res_score: 对应的得分列表。
    """
    if not scores:  # If there are no scores, initialize one
    ...
    # Calculate scores for sequence entities and keywords
    ...
    # Compute overall score using list comprehension and zip function
    ...
    # Reorder by score size
    sorted_pairs = sorted(zip(hybrid_scores, search_list), key=lambda x: x[0], reverse=True)

    # Extract the sorted search_list and score respectively
    res_score = [min(1, pair[0]) for pair in sorted_pairs if pair[0]>=threshold]  # Score is limited to 0-1
