...
def generate_chunks_bacth(chunks: list, batch_size=1000):
    ...
    batch_data = []
    # Use defaultdict to aggregate data
    aggregated_data = defaultdict(list)
    # Traverse the list and aggregate according to the chunk_current_num field
    temp_num = 0
    for item in chunks:
        temp_num += 1
    ...
    # Convert aggregated data to a normal dictionary for viewing
...
def add_milvus(user_id, kb_name, sub_chunk, add_file_name, add_file_path, kb_id="", milvus_url = ADD_URL):
    ...
    fail_count = 0
    error_reason = []
    # sub_chunk batch generator, group by chunk_current_num and generate batch data
    ...
    for batch in chunk_gen:
        ...
        try:
            ...
            if response.status_code != 200:
                ...
                # ========= Report an error and return directly to the end =======
                response_info['code'] = 1
    ...
    # print('add_milvus method calls interface to build database in batches, total batch: %s times, success: %s times, failure: %s times' % (batch_count, success_count, fail_count))
...
def update_milvus_content_status(user_id: str, kb_name: str, file_name: str, content_id: str, status: bool,
    ...
    if on_off_switch in [True, False]:  # The front end is passed the on_off_switch parameter
        data = {
            'userId': user_id,
            'kb_name': kb_name,
            'file_name': file_name,
...
def get_milvus_content_status(user_id: str, kb_name: str, content_id_list: list):
    ...
    """
    response_info = {'code': 0, "message": "成功"}
    # url = "http://localhost:30041/rag/kn/get_useful_content_status" # Temporary address
