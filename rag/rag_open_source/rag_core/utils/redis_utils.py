...
def delete_cache_by_prefix(redis_client, prefix):
    """
        ...
        """
    try:
        # Use the scan command to fuzzy query keys by prefix
        keys_to_delete = []
        cursor = '0'
        while cursor != 0:
...
# Method: Add a single piece of data based on user_id and entry id
def add_query_dict_entry(redis_client, user_id, term_entry, knowledgebase):
    ...
    for kb in knowledgebase:
        ...
        field = str(term_entry['id'])  # Use id as field
        value = json.dumps(term_entry)  # Convert entries to JSON string
        # Add to Redis hash table
        redis_client.hset(redis_key, field, value)
        print(f"已添加条目：{term_entry}")
        logger.info(f"已添加条目：{term_entry}")


# Method: Delete a single piece of data based on user_id and entry id
def delete_query_dict_entry(redis_client, user_id, term_entry_id, knowledgebase):
    ...
    for kb in knowledgebase:
        redis_key = f"query_dict:{user_id}:{kb}"
        field = str(term_entry_id)  # Use id as field

        # Delete the specified field from the Redis hash table
        redis_client.hdel(redis_key, field)
        print(f"已删除条目 id: {term_entry_id}")
        logger.info(f"已删除条目 id: {term_entry_id}")


# Method: Modify a single piece of data based on user_id and entry id
def update_query_dict_entry(redis_client, user_id, term_entry_id, new_term_entry, knowledgebase):
    ...
    for kb in knowledgebase:
        redis_key = f"query_dict:{user_id}:{kb}"
        field = str(term_entry_id)  # Use id as field
        value = json.dumps(new_term_entry)  # Convert entries to JSON string

        # Update fields in Redis hash table
        redis_client.hset(redis_key, field, value)
...
def update_chunk_labels(redis_client, kb_id, file_name, chunk_id, labels):
    ...
    try:
        ...
        # redis_client = get_redis_connection(redis_db=5) # Use fixed DB 5
        # Construct key
        hash_file_name = hashlib.md5(file_name.encode('utf-8')).hexdigest()  # Avoid special characters
        key = f"{kb_id}{hash_file_name}{chunk_id}"
        # Convert list of tags to JSON string storage
...
def delete_chunk_labels(redis_client, kb_id, file_name=""):
    ...
    try:
        # # Get Redis connection
        # redis_client = get_redis_connection(redis_db=5) # Use fixed DB 5
        # Use the scan command to find all matching keys
        if file_name:  # If a filename is specified, the filename is used to generate the prefix
            hash_file_name = hashlib.md5(file_name.encode('utf-8')).hexdigest()  # Avoid special characters
...
def get_all_chunk_labels(redis_client, kb_id):
    ...
    try:
        # # Get Redis connection
        # redis_client = get_redis_connection(redis_db=5) # Use fixed DB 5
        # Initialize a collection for deduplication
        unique_labels = set()
        # Use the scan command to find all matching keys
        cursor = "0"
        while cursor != 0:
            ...
            for key in keys:
                ...
                if value:
                    # Parse value in JSON format
                    chunk_data = json.loads(value)
                    # Add labels to the collection to remove duplicates
                    unique_labels.update(chunk_data.get("labels", []))
        # Convert a collection to a list and back
...
def delete_graph_vocabulary_set(redis_client, kb_id):
    ...
    try:
        ...
        # If the key does not exist, skip directly
        if not redis_client.exists(graph_key):
            pass
        else:
            # If the key exists, delete the old collection
...
def update_graph_vocabulary_set(redis_client, kb_id, elements_to_add=None, elements_to_remove=None):
    ...
    try:
        ...
        graph_key = f"graph_vocabulary_{kb_id}"
        if elements_to_add:
            redis_client.sadd(graph_key, *elements_to_add)  # Add element
        if elements_to_remove:
            redis_client.srem(graph_key, *elements_to_remove)  # Delete element
    except Exception as e:
...
if __name__ == "__main__":
    ...
    search_list = [{"id": "1", "value": 2}]  # Fixed duplicate key names
    ...
    # prefix = "13782ff0-5a73-4a51-8f7d-77015ee79aad^COMAC-Air Conditioning System^"
