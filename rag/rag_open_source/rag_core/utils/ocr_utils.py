...
def ocr_parser_native(add_file_path, ocr_model_id):
    ...
    # Read the file and record basic information
    try:
    ...
    # Logging request details
    logger.info(f"发起OCR请求 | URL: {wanwu_ocr_url} | 文件名: {file_name} | 模型ID: {ocr_model_id}")
    logger.debug(f"请求头信息: {files.keys()}")  # Debug level logging detailed header information
    ...
    try:
        ...
        if r.status_code == 200:
            # Logging a summary of successful responses
            logger.info(f"OCR请求成功 | 文件名: {file_name}")
            logger.debug(f"完整响应内容: {r.text}")  # Debug level logs full response

            try:
                ret_json = r.json()
                # Record the summary of key information after parsing
...
def get_page_data(page_num, add_file_path, ocr_model_id):
    ...
    try:
        ...
        # Construct request parameters (in line with formData requirements)
        files = {"file": (page_pdf_path, open(output_pdf_path, 'rb').read(), "application/pdf")}
        data = {"fileName": full_file_name}  # Explicitly pass the original file name
        ...
        try:
            ...
            # Parse the return result (conform to the new JSON structure)
            if ret_json.get("code") == 0:
                page_data = ret_json.get("data", [])
                # Add the current page number to the returned data (if the page_num returned by the interface is incorrect)
                for item in page_data:
                    item["page_num"] = [page_num]  # Make sure the page_num field is in list format
...
def ocr_parser(add_file_path, ocr_model_id):
    ...
    :param add_file_path: 文件路径
    """
    #logger.info("-----Visual photocopying scanned version pdf processing add_file_path=%s" % add_file_path)
    ...
    try:
        ...
        pdf_document.close()  # Close in advance to avoid file occupation

        with ThreadPoolExecutor(max_workers=OCR_MAX_WORKERS) as executor:  # Adjust max_workers to suit your needs
            ...
            for future in as_completed(futures):
                ...
                if page_data is not None:
                    for item in page_data:
                        if item["type"] == 'table':
                            md_text = hl2txt.handle(item["text"])  # Convert html_string to markdown
                            item["text"] = md_text
                        if item["type"] not in ['page-header', 'page-footer']:
                            if len(merged_data[page_num]['text'] + item["text"]) < MAX_SENTENCE_SIZE:
                                merged_data[page_num]["text"] += item["text"]  # Splice text
        ...
        # Add values ​​from merged_data to merged_list
