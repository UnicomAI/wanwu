...
def kafkal():
    while True:
        ...
        for message in consumer:
            ...
            # 文件导入时选择解析方式，默认勾选文字提取，可选光学识别ocr当多选时此参数默认为["text"],当勾选ocr时传：["text","ocr"] [EN] When importing files, select the parsing method. Text extraction is checked by default. Optical recognition OCR is optional. When multiple selections are made, this parameter defaults to ["text"]. When OCR is checked, pass: ["text", "ocr"]
            ...
            try:
                if not KAFKA_ENABLE_AUTO_COMMIT:
                    ...
                    # consumer.commit(offsets=offsets)  # 不需要使用这个提交 [EN] consumer.commit(offsets=offsets) # No need to use this commit
                    consumer.commit()
...
def parse_meta_data(docs, parse_rules):
    ...
    def parse_date_to_timestamp(date_str):
        # 常见的日期格式列表 [EN] List of common date formats
        date_formats = [
            # 英文格式 [EN] English format
            "%Y-%m-%d %H:%M:%S",
            "%Y-%m-%d %H:%M",
            "%Y-%m-%d",
            "%m/%d/%Y",
            ...
            # 其他常见格式 [EN] Other common formats
            "%Y.%m.%d",
            "%Y.%m.%d %H:%M:%S",
        ]

        # 预处理：处理一些特殊情况 [EN] Preprocessing: Handling some special cases
        processed_str = date_str.strip()
        processed_str = processed_str.replace(" ", "")

        # 尝试每种格式 [EN] try every format
    ...
    for item in parse_rules:
        pattern = item["rule"]
        if pattern:
            # 通过rule提取元数据 [EN] Extract metadata through rules
            meta_datas = []
            #反转义 [EN] Anti-escaping
            ...
            if meta_datas:
                ...
                if item["value_type"] == "time":
                    try:
                        ...
                        # 提取的时间类型的元数据不支持转成unix timestamp [EN] The extracted time type metadata does not support conversion to unix timestamp.
                        continue
            else:
                #提取不到元数据 [EN] Unable to extract metadata
                continue
        else:
            item["value"] = str(item["value"])

        # 如果item["rule"] 为空，item["value"]是固定值 [EN] If item["rule"] is empty, item["value"] is a fixed value
        result.append(item)
...
def add_files(user_id, kb_name, file_name, object_name, file_id,
    ...
    try:
        ...
        if not download_status:
            ...
            # 转换文件格式 [EN] Convert file format
            base_filename, file_extension = os.path.splitext(file_name)  # 分离文件名和后缀 [EN] Separate filename and suffix
            master_control_logger.info(f"base_filename={base_filename} file_extension={file_extension}")
            if "model" in split_config.parser_choices and file_extension in [".doc", ".docx", ".pptx"]:  # 先判断是否模型解析 [EN] First determine whether the model is analyzed
                convert_office_format_map = {".doc": "pdf", ".docx": "pdf", ".pptx": "pdf"}
                target_format = convert_office_format_map[file_extension]  # 获取目标格式 [EN] Get target format
    ...
    try:
        ...
        if res_filename:  # 需要传递转换后的 文件路径 [EN] Need to pass the converted file path
        ...
        with open("./data/%s_chunk.txt" % file_name, 'w', encoding='utf-8') as chunks_file:
            for item in chunks:
                if "download_link" not in item["meta_data"]:
                    item["meta_data"]["download_link"] = download_link  # 添加file下载链接 [EN] Add file download link
                if res_filename and "file_name" in item["meta_data"]:  # 如果有转换后的文件，则替换回原来文件名 [EN] If there is a converted file, replace it back with the original file name.
                    item["meta_data"]["file_name"] = file_name
                # 存储 BUCKET 和 object_name [EN] Store BUCKET and object_name
                item["meta_data"]["bucket_name"] = BUCKET_NAME  # 添加文件桶名 [EN] Add file bucket name
                item["meta_data"]["object_name"] = object_name  # 添加文件下载对象名 [EN] Add file download object name
        ...
        if "Error loading" in repr(e):  # 文件不可用 [EN] File not available
            logger.error('文档切分失败' + "user_id=%s,kb_name=%s,file_name=%s" % (user_id, kb_name, file_name))
