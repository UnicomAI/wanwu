...
# Allows nested use of asyncio within existing event loops
nest_asyncio.apply()
...
def clean_text(text):
    ...
    patterns = [
        r'\xa0+', r'\u3000', r'\t+', r'\r+', r'\n+',   # Clear special whitespace characters and multiline newlines
        r'<[/]?b>|&gt;|&lt;'                        # Clear HTML tags
    ]
...
def clean_list(data_list, black_domains=None):
    ...
    if black_domains is None:
        # If no blacklist website list is passed in, bendibao.com is used by default (the weather forecast is inaccurate)
    ...
    return [
        ...
        # Skip elements in links containing any blacklisted domain name
...
# Main process: step1: Bing search
...
def bing_cleaned_search(query, result_len=BING_RESULT_LEN, days_limit=-1,setLang ="zh-hans"):
    ...
    # Check whether Bing search related environment variables are set
    if not (BING_SEARCH_URL and BING_SUBSCRIPTION_KEY):
        # If the relevant environment variables are not set, a prompt message will be returned.
        return [{"type":"SE",
                 "snippet": "please set BING_SUBSCRIPTION_KEY and BING_SEARCH_URL in os ENV",
                 "title": "env info is not found",
                 "link": "https://python.langchain.com/en/latest/modules/agents/tools/examples/bing_search.html"}]
    
    # Initialize the Bing searcher object
...
#     print(json.dumps(bing_result,ensure_ascii=False))
    ...
    # If days_limit =-1, it means no date filtering is performed and the cleaned search results are returned directly.
    if days_limit == -1:
        return clean_list(bing_result)[:result_len]

    # Get the time zone of East Eighth District (China Standard Time)
    tz = pytz.timezone('Asia/Shanghai')

    # Get the current Dongba District time
    today = datetime.datetime.now(tz)

    # Calculate the date before the specified number of days as the cut-off date for filtering
    cutoff_date = today - datetime.timedelta(days=days_limit)

    # Filter search results that match the criteria (including results with a blank publication date or within a specified number of days)
    recent_articles = []
    
    for item in bing_result:
        # Gets the publication date field, returning an empty string if it does not exist
        date_published_str = item.get('datePublished', '')  
        
        if not date_published_str:
            # If datePublished is empty or missing, add this entry to the filter results
            recent_articles.append(item)
        else:
            try:
                # Parse the release date string into a datetime object and convert it to East Eighth District time
                published_date = datetime.datetime.strptime(date_published_str, "%Y-%m-%d").replace(tzinfo=pytz.UTC).astimezone(tz)
                
                # If the release date is within the specified number of days, add the entry to the filter results
                if published_date > cutoff_date:
                    recent_articles.append(item)
            except ValueError:
                # If the date format is incorrect, print a warning and include the entry in the filter results
    ...
    # Clean and return filtered search results, limiting the number of results to result_len
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        try:
            ...
            async with session.post(parser_url, json=payload, headers=headers) as response:
                ...
                elapsed_time = (datetime.datetime.now() - start_time).total_seconds()
                if not docs:
                    # logger.info(f"Resolved as empty: {query[:QUERY_SHORT]} ---> Time consuming: {elapsed_time}s ---> {bing_single_item['link']}")
                    return []

                logger.info(f"解析【正常】:{query[:QUERY_SHORT]}  ---> 耗时: {elapsed_time}s ---> 个数：{len(docs)}---> {bing_single_item['link']}")
                # logger.info(f"Parse [normal]:{docs}")
            ...
            elapsed_time = (datetime.datetime.now() - start_time).total_seconds()
            # logger.info(f"Parse timeout: {query[:QUERY_SHORT]} ---> Time consuming: {elapsed_time}s ---> {bing_single_item['link']}")
        except Exception as e:
            pass
...
async def process_url_with_fallback(bing_single_item, query, sentence_size, time_out):
    async def fetch_parser(is_google):
        try:
            ...
            return []  # The task is canceled and an empty list is returned.
        except Exception as e:
    ...
    try:
        while tasks:
            ...
            for task in done:
                result = await task
                if result:  # If the task returns valid results
                    # Cancel all other outstanding tasks
                    for p in pending:
                        p.cancel()
                    
                    await asyncio.gather(*pending, return_exceptions=True)  # Make sure the task is completely terminated
                    
                    return result  # Only return the first successful result

            # If all `done` tasks return empty data, continue to wait for the remaining tasks.
            tasks = list(pending)

    finally:
        # Ensure all tasks are canceled to avoid resource leakage
        for task in tasks:
            task.cancel()
        await asyncio.gather(*tasks, return_exceptions=True)

    return bing_single_item  # If all tasks fail, return the original result of bing
    # return []
    ...
    try:
        for completed_task in asyncio.as_completed(tasks):
            try:
                ...
                return aggregated_results  # Make sure to return existing results
            except Exception as e:
        ...
        for task in tasks:
            task.cancel()
        await asyncio.sleep(0)  # Give up control and ensure cancellation takes effect
        await asyncio.gather(*tasks, return_exceptions=True)
...
def handle_sub_query(
    ...
    #---- URL content capture and analysis
    start_time = datetime.datetime.now()
    ...
    if sub_results:
        if isConcurrent:
            # When calling concurrently, add an arrow after sub_query to optimize the prompt to LLM.
            for sub_result in sub_results:
                sub_result["sub_query"] = sub_query + "-> "
    else:   
        # If the URL parsing interface is empty, the original search engine results will be used.
        sub_results.extend(hybrid_search_list)
    
    if is_rerank:        
        # Use new reorder function
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            try:
                ...
                traceback.print_exc()
                # Throw an exception to ensure the caller knows an error has occurred
                raise
...
def combine_search_list(search_list):
    ...
    for item in search_list:
        ...
        if link in combined_dict:
            # Merge snippets, use newlines to connect
            combined_dict[link]['snippet'] += '\n' + item['snippet']
            
        else:
            # Initialize dictionary items, copying current elements to avoid modifying the original data
            combined_dict[link] = item.copy()

    # Convert the merged dictionary back to list form
...
def hybrid_clean_search(
    ...
    # Create a thread pool to perform emb and bm25 concurrent sorting on network query results.
    bing_search_list=[]
    bocha_search_list=[]

    
    with concurrent.futures.ThreadPoolExecutor() as executor:
        # Prepare parameters for two tasks
...
def deduplicate_by_title(search_list):
    ...
    for item in search_list:
        ...
        if title in combined_dict:
            # If the title already exists, compare the snippet lengths and keep the longer one.
...
def get_bing_multi_search_result(query,bing_top_k,bing_time_out,bing_target_success,bing_result_len,model,search_url,search_key,search_rerank_id,days_limit=-1,auto_citation=False):
    ...
    # When controlling multiple subqueries total_top_k: =sub_query_num * top_k_ration * BING_TOP_K
    if sub_query_num <= 3:
        top_k_ration = 1
...
async def async_search(query, bing_top_k,bing_time_out,bing_target_success,bing_result_len,model,search_url,search_key,search_rerank_id,days_limit=-1, auto_citation=False):
    # Assume get_bing_multi_search_result is synchronous and needs to run in the thread pool
    loop = asyncio.get_running_loop()
    result = await loop.run_in_executor(
        None,  # Default thread pool
        get_bing_multi_search_result,  # Synchronous function
        query,  # Parameter 1
        bing_top_k,
        bing_time_out,
        bing_target_success,
        ...
        search_key,
        search_rerank_id,
        days_limit,  # Parameter 2
        auto_citation# Parameter 3
    )
    return result
...
def start_async_search(loop, query,bing_top_k,bing_time_out,bing_target_success,bing_result_len,model,days_limit, auto_citation,search_url,search_key,search_rerank_id):
    ...
    # print("The asynchronous task has been started and will not block subsequent code")
    return task

# Example for running an asynchronous function
if __name__ == "__main__":
    ...
    # query = "Where will this year's World Weightlifting Championships be held?"
    # query = "How many concerts has Deng Ziqi held?"
    # query = "Tencent US stock price"
    # query = "Alibaba US stock price"
    # query = "Baidu US stock price"
    # query = "Why is the 2024 China-Africa Forum not held in Beijing, but in Beijing?"
