...
class ReActChat(FnCallAgent):
    ...
    def _run(self, messages: List[Message],lang: Literal['en', 'zh'] = 'en', **kwargs) -> Iterator[List[Message]]:
        ...
        qa_type = 20
        thought_inference = ""
        param_des = []  ###Save the parameter descriptions corresponding to different calling models to facilitate identifying which parameters are missing, so as to call the large model for dialogue tracking
        function_items =  []
        ...
        try:
            ####1. The output format of the yuanjing_function_call model is inconsistent with other architectures and needs to be processed separately------directly called
            ...
            if self.action_type == "function_call":
                    ...
                    if response.status_code == 200 :
                        ...
                        if res["data"]["choices"][0]["finish_reason"] == "tool_calls":
                            ...
                            if action.upper() != "NONE":
                                ...
                                ###Get detailed parameter description
                                ...
                                ###Determine whether function_call parameter recognition is missing
                                ...
                                if action_input and (not any(v is None  or  (v and v.strip() == "") or (not v) for v in action_input.values())) and  all(element in keys_identify for element in matches):
                                    ...
                                    ## Do not use words such as "based on the information you provided", "based on the information provided", "based on reference information", etc. in the answer.
                                    ## Please do not use markdown format.
                                    ## Please answer concisely and accurately tell the user which parameters are missing and the parameter requirements that need to be entered.
                                    '''   
                                    ###Call the large model to answer
                                    messages = []
                                    subjson = {}
                                    subjson["role"] = "user"
                            ...
                            ###Determine whether it is a further inquiry into the question
                ...
                ###Define detailed list of plugins
                ...
                while num_llm_calls_available > 0:
                    ...
                    for output in output_stream:
                        if output and output[-1].content:
                            ...
                            if "<TOOL>" in response.upper():
                                # logger.info(f"-----deepseek truncation extraction: stop_token effect------")
                                ...
                                if "</TOOL>" in response and ("ACTION INPUT" in response_new.upper() and "}" in response_new.upper()):
                                    break
                            # ####If it is directly the answer line, no verification is needed and the result is output directly.
                            ...
                            #     # logger.info(f"The final output result of action is: {result}")
                            #     first_index += 1

                            else:
                                # logger.info(f"-----Non-deepseek truncation extraction: stop_token effect------")
                    ...
                    ####Large model reports an error, exit directly
                    if not response:
                    ...
                    ###Extract tool call results after eliminating the thinking process
                    response = re.sub('<tool>.*?</tool>', '', response, flags=re.DOTALL)
                    response = re.sub('<think>.*?</think>', '', response, flags=re.DOTALL | re.IGNORECASE)
                    ##Structured extraction of parameters for large model recognition: compatible with Yuanjing model and ds model (output content is inconsistent)
                    counts = {
                    ...
                    ####### Determine whether the number of extracted parameters is complete and valid
                    ###1. Identify the extracted parameters
                    ...
                    ####2. Identify the parameter names and parameter descriptions of the original API
                    if action and all(item in function_items for item in action_results['func_name']):
                        ...
                        ##### Index location and extraction of plug-in
                        ...
                        ####Index position and extraction of function_to_call
                        else:
                    ...
                    #####3. Compare the difference between original and recognized action and action_input: compatible with Yuanjing and ds return formats
                    ####1) Effective calling link: Only when the output contains Action and the parameters are not empty, further differences need to be compared, otherwise the output link will be used directly.
                    if ("Action:" in response or "Action" in response ) and action_results['func_name']:
                        if not all(item in function_items for item in action_results['func_name']):
                            ...
                            ####qwen_agent allows react to reflect one more step
                            count_qs += 1
                            if count_qs< 3:
                                ...
                                continue
                    ####Output link: three situations in total
                    ###1) The large model can answer by itself
                    ###2) The result after calling the plug-in can answer user questions
                    ###3) The result after calling the plug-in cannot answer the user's question, but is a further supplement to the user's question (effective when the user's question is unclear)
                    ###4) When unable to answer, code returns 1
                    else:
                        if t == 1:
                            action_results['action_code'] = 1
                        ...
                        if "Final Answer" in can_answer_text or "Final Answer:" in can_answer_text:
                            ###Eliminate the thinking process of think
                    ...
                    ####Complete parameter extraction and start function call
                    ###1. Integrate action and input parameters [Specifically, the process of modelscope_agent is: ACTION extraction, ACTION_INPUT extraction, and call invocation in three steps; the non-modelscope_agent process is in two steps: ACTION+ACTION_INPUT extraction, call invocation]
                    ...
                    ###2. Complete ACTION extraction and start streaming output.
                    if "<tool>" not in total_result and action:
                        total_result += "<tool>"  
                        if t == 1:
                            yield [Message(role=ASSISTANT, content=f"\n--start--\n")]
                    ...
                    ###3. modelscope_agent requires special processing to extract ACTION_INPUT
                    if self.action_type == "modelscope_agent" and func_names:
                        if t<=1: 
                            ...
                            ###Remove the identified actions and parameters
                            func_names.pop()
                            # func_params.pop()
                            continue
                    
                    ###4. Complete ACTION_INPUT extraction and start streaming output
                    if action and action_input:
                        yield [Message(role=ASSISTANT, content=f"\n\n```请求参数：\n{action_input[0]}\n```\n\n")]
                        

                    ###5. Identify the type of ACTION. If it is function_call, it will be identified as qa_type is 21. If it is action, it will be identified as qa_type is 20.
                    ###function_call does not need to be called, it directly returns ACTION and ACTION_INPUT
                    ...
                    ###6. Continue to call functions or output results.
                    ####1) When not action, the table name output result
                    ###2) Action indicates that _call_tool will continue to be executed.
                    if action_results['action_code'] == 0:
                        ...
                        if not action:
                            ...
                            if qa_type == 20:
                                ...
                                action = action[-1]
                                action_input = action_input[-1] if action_input else {}   ###There are cases where the plug-in does not require required parameters.
                                ...
                                if is_use_api_output and api_stream:
                                    ...
                                    try:
                                        ...
                                        ####API internal service exception
    ...
    def _prepend_react_prompt(self, messages: List[Message], lang: Literal['en', 'zh']) -> List[Message]:
        ...
        if self.action_type == "modelscope_agent":
            for f in self.function_map.values():
                function = f.function
                # logger.info(f"The currently parsed function is: {function}")
            ...
            for fn in self.function_calls_list:
                # logger.info(f"The current function_list is: {fn}")
                name = fn.get('function_name', None)
                name_for_human = fn.get('name_for_human', name)
                name_for_model = fn.get('name_for_model', name)
                assert name_for_human and name_for_model
...
def find_tools(text: str,num: int) -> Tuple[bool, str, str, str]:
    ###Adapt non-deepseek model output
    ...
    #####Output format compatible with deepseek
    text = text.replace("https//", "https://")  ###Adapt the link output of large models
    text = text.replace("https://192.168.0.218081","https://192.168.0.21:8081")
    text = text.replace("http//", "http://")
    text = text.replace("minio-wanwu9000","minio-wanwu:9000")

    ###Adapt the action and parameter output format of deepseek series models
    ...
    for _ in range(num):
        ...
        if 0 <= i < j:  
            ...
            ###Regularize extraction function_name (remove all punctuation marks and whitespace characters)
            ...
            ###Regularization extraction function_args (extract the content between {})
...
def parse_tool_selection_response(response_text: str, functions: List[OpenAPIPluginTool]):
        ...
        for function in functions:
            # Likewise, convert tool names to lowercase for matching
            if isinstance(function, OpenAPIPluginTool):
                if function.name.lower() in response_text_lower:
                    return function.name  # Returns the first matching tool name
                
            else:
                if function['function_name'].lower() in response_text_lower:
                    return function['function_name']

        # If no matching tool name is found, returns None or the default value
        return None    
