...
def bocha_cleaned_search(query,search_url,search_key, result_len):
    ...
    headers = {
        'Authorization': search_key,  # 请替换为你自己的API密钥 [EN] Please replace with your own API key
        'Content-Type': 'application/json'
    }

    try:
        # 发起请求并检查状态码 [EN] Make a request and check the status code
        response = requests.request("POST", url, headers=headers, data=payload)
        response.raise_for_status()  # 如果返回的状态码不是 2xx，会抛出异常 [EN] If the returned status code is not 2xx, an exception will be thrown
        response_data = response.json()  # 尝试解析 JSON [EN] Try to parse JSON
        
        # print("返回数据:", response_data) [EN] print("Return data:", response_data)
        # print()

        # 映射返回结果 [EN] Mapping returns results
        ...
        # 特别处理不同的 HTTP 错误码 [EN] Special handling of different HTTP error codes
