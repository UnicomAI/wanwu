...
class CustomWebLoader(WebBaseLoader):
    ...
    def _scrape(
        ...
        # Select parser based on URL suffix
        if parser is None:
            if url.endswith(".xml"):
                parser = "xml"
            else:
        ...
        decoded_text = None
        replacement_threshold = 0.05  # Replacement character ratio threshold
        ...
        # 0. Try to detect the encoding from the meta tag, giving priority to the encoding and decoding specified by meta.
        ...
        # 1. If the user provides an encoding list, traverse all candidates and select the result with the highest proportion of Chinese characters.
            ...
            if isinstance(self.encoding, (list, tuple)) and not isinstance(self.encoding, str):
                for enc in self.encoding:
                    ...
                    if candidate_text is not None:
                        ...
                        # If the candidate decoding has a higher proportion of Chinese, record that encoding
        ...
        # 2. Use chardet to automatically detect encoding
        ...
        # 3. Try default utf-8 decoding (no encoding parameters specified)
        if decoded_text is None:
            candidate_text = try_decode(content_bytes, None)
            if candidate_text is not None:
                decoded_text = candidate_text

        # 4. If none of the above methods are successful, decide whether to use apparent_encoding according to the autoset_encoding configuration.
        if decoded_text is None:
