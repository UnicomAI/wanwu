...
# 可控版文生图 [EN] Controllable version of Wenshengtu
@advanced_timing_decorator() 
def upload_base64_to_url(image_bs64_list):
    ...
    try:
        for image_bs64 in image_bs64_list:       
            file_extension = '.jpg'  # 假设我们知道文件应该是JPEG图像 [EN] Assuming we know the file should be a JPEG image
        ...
        logger.info(f"Minio没有生成url: {str(e)}")  # 记录错误日志 [EN] Record error log
        msg = str(e)
        return img_url_list,msg
...
def req_txt2img_plus(query,images_per_prompt=1,height=1024,width=1024):
    ...
    try:
        ...
        logger.error("没有成功生成图片呢: %s", e)  # 记录错误日志 [EN] Record error log
        msg = str(e)
...
# 图文问答plus [EN] Picture and text question and answer plus
@advanced_timing_decorator() 
def unicom_vqa_stream(image_url, prompt,messages):
    ...
    if bucket_type == "MINIO":
        try:
            ...
            image_response.raise_for_status()  # 确保请求成功 [EN] Make sure the request is successful
        except requests.RequestException as e:
            logger.error("Image download error: %s", e)  # 记录错误日志 [EN] Record error log
            return "No answer found due to image download error"
        # 使用获取的图片内容创建一个临时的BytesIO对象 [EN] Create a temporary BytesIO object using the obtained image content
    ...
    # 准备请求数据 [EN] Prepare request data
    ...
    try:
        ...
        for line in response.iter_lines(decode_unicode=True):
            ...
            # content='智能' additional_kwargs={} response_metadata={} id='bece9cbc-e38f-11ef-a1b4-b64ea7f91903' usage_metadata={'input_tokens': 36, 'output_tokens': 7, 'total_tokens': 43} [EN] content='smart' additional_kwargs={} response_metadata={} id='bece9cbc-e38f-11ef-a1b4-b64ea7f91903' usage_metadata={'input_tokens': 36, 'output_tokens': 7, 'total_tokens': 43}
        ...
        logger.error("API error: %s", e)  # 记录错误日志 [EN] Record error log
        print(e)
...
#     # 第一步：生成视频 [EN] # Step one: Generate video
#     resource_id = request_text_to_video(prompt)

#     # 等待5秒：视频生成需要3-5秒 [EN] # Wait 5 seconds: video generation takes 3-5 seconds
#     time.sleep(4)

#     # 第二步：获取视频详细信息 [EN] # Step 2: Get video details
...
#     # 第三步：上传视频到 MinIO [EN] # Step 3: Upload video to MinIO
...
@advanced_timing_decorator()    
def generate_and_upload_video(prompt):
    # 第一步：生成视频 [EN] Step 1: Generate video
    resource_id = request_text_to_video(prompt)
    # 如果没有生成成功，则返回失败信息 [EN] If the generation is not successful, failure information is returned.
    if not resource_id:
        return json.dumps({"code": 1, "message": "Failed to generate video", "url": ""})

    # 第二步：轮询获取视频详细信息，最长等待30秒 [EN] Step 2: Poll to obtain video details, wait up to 30 seconds
    ...
    while time.time() - start_time < 30:
        ...
        if video_base64:
            break
        time.sleep(1)  # 每秒检查一次 [EN] Check every second

    # 如果获取视频信息失败，则返回失败信息 [EN] If the acquisition of video information fails, failure information will be returned.
    if not video_base64:
...
def req_code_interpreter(query,need_file=False,upload_file_url='',history=[]):
    ...
    # 发起流式请求 [EN] Make a streaming request
...
def is_query_sen(query):
    ...
    try:
        ...
        logger.error("is_query_sen error: %s", str(e))  # 记录错误日志 [EN] Record error log
        return None
...
def req_action(query,plugin_list = None,function_calls_list = None,action_type = "qwen_agent",history = []):
    ...
    try:
        ...
        logger.error("req_action error: %s", str(e))  # 记录错误日志 [EN] Record error log
...
def is_kb_belong_to_user_v2(kn_list, userId):
    ...
    # url = "http://192.168.0.214:7801/rag/list-knowledge-base"  #负载地址 [EN] url = "http://192.168.0.214:7801/rag/list-knowledge-base" #Load address
    url = config["MODELS"]["default_list_kb_url"]
    
    # 初始化返回值 [EN] initialization return value
    code = 0
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # 使用集合判断 kn_list 是否是 user_kb_list 的子集 [EN] Use a set to determine whether kn_list is a subset of user_kb_list
            kn_set = set(kn_list)
        ...
        code = -1
        logger.error("is_kb_belong_to_user_v2 error: %s", str(e))  # 记录错误日志 [EN] Record error log
        print(f"An error occurred: {e}")  # 用于调试，可删除 [EN] Used for debugging and can be deleted
        msg = str(e)
        return code, msg
...
def req_kb_search_v2(question, kn_userId, kn_name, threshold, topK,auto_citation=False):
    ...
    prompt = ''
    search_list = []
    # url = "http://192.168.0.214:7801/rag/search-knowledge-base"  # 负载地址 [EN] url = "http://192.168.0.214:7801/rag/search-knowledge-base" # Payload address
    ...
    try:
        with open(file_path, "rb") as f:
            response = requests.post(
                ...
                headers=headers,
                files={"file": f},
                verify=False  # 建议根据需求配置 SSL 验证 [EN] It is recommended to configure SSL verification according to needs
            )
        if response.status_code == 200:
            data = response.json()
            ...
            # print(f"上传失败，状态码：{response.status_code}") [EN] print(f"Upload failed, status code: {response.status_code}")
...
def req_chat_doc(query,file_url):  
    ...
    DOC_PROMPT_TEMPLATE = '''
        ...
        【用户问题】{question}
        '''
          # - 如果用户问题是"这是什么"、"文档内容"、"这个文件的内容是什么"等之类问题时，输出文档内容； [EN] - If the user's question is "what is this", "document content", "what is the content of this file", etc., output the document content;
...
####统计类问题、非统计类问题分类 [EN] ###Classification of statistical problems and non-statistical problems
...
def qa_stats_cls_by_llm(query,model= "unicom-70b-chat"):
    ...
    pattern = r',\s*"?REASON.*'         # 匹配 , 或 "REASON 后面的所有内容 [EN] Matches everything after , or "REASON
    result = re.sub(pattern, '}', response)
    result_json = extract_json(result)
...
###chat_excel服务 [EN] ##chat_excel service
...
def req_chat_excel(data):
    ...
    try:
        ...
        logger.error("chatexcel_server API error: %s", e)  # 记录错误日志 [EN] Record error log
        return None
...
def req_chat_gui(data):
    ...
    try:
        ...
        logger.error("req_chat_gui API error: %s", e)  # 记录错误日志 [EN] Record error log
        return None
...
def intent_cls_by_memory(query,history_messages,model="unicom-70b-chat"):
    ...
    ```   
     ...
     输出要求：
     ## 如果完全无法从中得到答案，请回答 "0",并说明原因。如果可以从中得到答案，请回答"1"。 [EN] # If you cannot get an answer from it at all, please answer "0" and explain the reason. If you can get an answer from it, please answer "1".
     ## 请用json格式回答，key为TYPE和REASON。''' [EN] # Please answer in json format, the keys are TYPE and REASON. '''
...
def answer_by_memory(query,history_messages,stream):
    ...
    ```   
     ...
     输出要求：
     ## 答案中不要出现"根据对话历史"字样。 [EN] # Do not include the words "based on conversation history" in the answer.
  '''
...
if __name__ == "__main__":
    ...
    # print(req_unicom_llm_chat2(messages,["原因"])) [EN] print(req_unicom_llm_chat2(messages,["reason"]))

    # print(get_intent_cls("这是什么")) [EN] print(get_intent_cls("What is this"))
    # url = "https://maas-gz.ai-yuanjing.com/minio/download/api/public/abcc1e25-751f-49b7-9955-b8a3cbf57e78-2fe925ba-4341-438f-912e-8d3d3c692b9f@民法.pdf" [EN] url = "https://maas-gz.ai-yuanjing.com/minio/download/api/public/abcc1e25-751f-49b7-9955-b8a3cbf57e78-2fe925ba-4341-438f-912e-8d3d3c692b9f@civillaw.pdf"
    # query = "公司IT系统数据安全工作的直接责任单位是哪个部门" [EN] query = "Which department is directly responsible for the company's IT system data security work?"
    # req_chat_doc(query,url)
    # print(redis_client.get_value(url))
    
    # query = "帮我合成意图语音，内容是：四川美食可多了，有麻辣火锅、宫保鸡丁、麻婆豆腐、担担面、回锅肉、夫妻肺片等，每样都让人垂涎三尺!" [EN] query = "Help me synthesize the intended speech. The content is: There are many Sichuan delicacies, including spicy hot pot, Kung Pao chicken, Mapo tofu, dandan noodles, twice-cooked pork, couple's lung slices, etc., each one is mouth-watering!"
...
#     file_path = "dy英语视频.MP4" [EN] file_path = "dyEnglish video.MP4"
    
#     print(upload_file_to_minio(file_path))
              
    # query = "唐朝诗仙是谁" [EN] query = "Who is the Poet of the Tang Dynasty?"
...
#     if download_link:
#         print(f"文件已上传，下载链接为：{download_link}") [EN] print(f"The file has been uploaded, the download link is: {download_link}")
#     else:
#         print("文件上传失败。") [EN] print("File upload failed.")
  
    #-----vqa-----
    # pic_url = "https://maas-gz.ai-yuanjing.com/minio/download/api/public/tmpoy4ovbqi.jpg"
    # prompt = "这是什么" [EN] prompt = "What is this"
    ...
    #-----rag-------
    # prompt,search_list= req_kb_search_v2("2025亚冬会口号", "b5d139b5-901b-4ae0-8fd5-8c57df77476e","AsianWinterGame", threshold=0.0, topK=5) [EN] prompt,search_list= req_kb_search_v2("2025 Asian Winter Games slogan", "b5d139b5-901b-4ae0-8fd5-8c57df77476e", "AsianWinterGame", threshold=0.0, topK=5)
    # print(prompt)
    # print(search_list)
    ...
    # ----langchain LLM ----
    # query = "她有哪些代表作" [EN] query = "What are her masterpieces"
    # query = '赵丽颖的生日是那一天？' [EN] query = 'When is Zhao Liying's birthday? '
    
    # messages =[
    #     {"role": "user", "content": "你是谁"}, [EN] {"role": "user", "content": "Who are you"}, 
    #     {"role": "assistant", "content": "我是元景，一个人工智能聊天机器人。我由联通开发，专门设计用来帮助人们解答问题、提供信息和进行对话。如果你有任何疑问或需要帮助，都可以随时问我。"}, [EN] {"role": "assistant", "content": "I am Yuan Jing, an artificial intelligence chatbot. I was developed by China Unicom and specially designed to help people answer questions, provide information and have conversations. If you have any questions or need help, you can always ask me."}, 
    #     {"role": "user", "content": "赵丽颖是谁"}, [EN] {"role": "user", "content": "Who is Zhao Liying"}, 
    #     {"role": "assistant", "content": "赵丽颖是中国著名女演员，1987年10月16日出生于河北省廊坊市，毕业于廊坊市电子信息工程学校。\n\n她在2006年因获得雅虎搜星比赛冯小刚组冠军而进入演艺圈，并随后出演了多部知名电视剧和电影，如《新还珠格格》中的晴儿，《陆贞传奇》中的陆贞等角色，这些作品使她逐渐走红并积累了大量粉丝。\n\n赵丽颖以其精湛的演技和甜美的形象赢得了观众的喜爱，成为了中国娱乐圈中备受瞩目的明星之一。除了演艺事业外，她还积极参与公益活动，展现了其社会责任感和公益精神。"}, [EN] {"role": "assistant", "content": "Zhao Liying is a famous Chinese actress. She was born in Langfang City, Hebei Province on October 16, 1987. She graduated from Langfang Electronic Information Engineering School.\n\nShe entered the entertainment industry in 2006 when she won the championship of Feng Xiaogang's group in the Yahoo Star Search Competition, and subsequently appeared in many well-known TV series and movies, such as Qing'er in "The My Fair Princess", "Lu Zhen" "Legend" and other roles, these works have made her gradually popular and accumulated a large number of fans. \n\nZhao Liying has won the love of the audience with her superb acting skills and sweet image, and has become one of the high-profile stars in the Chinese entertainment industry. In addition to her acting career, she also actively participates in charity activities, demonstrating her sense of social responsibility and public welfare spirit.
    #     # {"role":"user","content":query},
    # ]

    # messages = [{'role': 'user', 'content': '你是谁'}] [EN] messages = [{'role': 'user', 'content': 'Who are you'}]
    ...
    # query =  "生成一个柱状图，纵轴数据分别为100,200,300，横轴是2022,2023,2024？" [EN] query = "Generate a histogram with the vertical axis data being 100, 200, 300 and the horizontal axis being 2022, 2023, 2024?"
    # query =  "编程生成一个柱状图，纵轴数据分别为100,200,300，横轴是2022,2023,2024,保存到新文件" [EN] query = "Programming to generate a histogram, the vertical axis data are 100, 200, 300, the horizontal axis is 2022, 2023, 2024, save to a new file"
...
# #     file_name = "interpreter_test.txt"
    ...
    # query =  "编写程序将图片转为黑白的。" [EN] query = "Write a program to convert pictures to black and white."
    # query = "编写一个冒泡排序呈现" [EN] query = "Write a bubble sort presentation"
    # datajson = {}
    ...
    # query = '都是这么收费的？' [EN] query = 'Is this how much it charges? '
...
#     print(result)
    ...
    # video_base64 = get_video_base64(resource_id)
    # print("SSS:\n",video_base64,"\nHHH")
    # prompt = "绘制一个视频，随风。摆动的柳树" [EN] prompt = "Draw a video, with the wind. Swinging willow tree"
    # result = generate_and_upload_video(prompt)
    # print(result)
    # query = '柯基' [EN] query = 'Corgi'
    ...
    # result = generate_and_upload_video_img2vid(pic_url,"生成一段视频") [EN] result = generate_and_upload_video_img2vid(pic_url,"Generate a video")
    # print(result)

    
#     query = """构造参数的提示词为：选中的工具是 action_getWeatherNow，需要以下参数： [EN] The prompt word for query = """ construction parameters is: the selected tool is action_getWeatherNow, which requires the following parameters:
# 参数 location: 查询的地点，可以是城市名、邮编等。 [EN] Parameter location: The location of the query, which can be a city name, zip code, etc.
# 请根据以下用户的问题，生成所需的参数，参数请以json格式输出：今天北京天气怎么样""" [EN] Please generate the required parameters based on the following user questions. Please output the parameters in json format: How is the weather in Beijing today"""
...
#     print(response.headers["Content-Type"])
    ...
    # print(get_intent_cls("明天天气")) [EN] print(get_intent_cls("Tomorrow's weather"))
    # try:
