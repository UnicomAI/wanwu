...
class CustomTokenizer:
    # 使用类变量来存储模型实例，确保每个模型只有一个实例 [EN] Use class variables to store model instances, ensuring there is only one instance of each model
    _instances = {}
    # 支持的模型列表 [EN] Supported model list
    _supported_models = [
        "deepseek-r1",
    ...
    # 默认字符到token的转换比例 [EN] Default character to token conversion ratio
    ...
    def __init__(self, tokenizer_dir: str = "/agent/agent_open_source/utils/tokenizers", model_name: str = "deepseek-v3", device: str = None):
        ...
        if not hasattr(self, 'tokenizer'):  # 确保只初始化一次 [EN] Make sure to initialize only once
            self.tokenizer = None
            self.device = None
    ...
    def count_tokens(self, text: Union[str, List[str]]) -> int:
        ...
        if not self.is_supported_model:
            # 对于不支持的模型，使用字符长度估算 [EN] For unsupported models, use character length estimates
            ...
            # 对于支持的模型，使用tokenizer计算 [EN] For supported models, use tokenizer calculations
            if isinstance(text, str):
        ...
        # logger.info(f"计算token数量耗时: {time.time() - start_time:.4f}秒") [EN] logger.info(f"Calculating the number of tokens takes: {time.time() - start_time:.4f} seconds")
        return result
    ...
    def encode(self, text: str) -> List[int]:
        ...
        if not self.is_supported_model:
            # 对于不支持的模型，返回一个模拟的token列表 [EN] For unsupported models, return a simulated token list
    ...
    def decode(self, token_ids: List[int]) -> str:
        ...
        if not self.is_supported_model:
            # 对于不支持的模型，返回一个警告信息 [EN] For unsupported models, return a warning message
    ...
    # 删除实例方法 is_model_supported [EN] Remove instance method is_model_supported
    ...
    def truncate_text(self, text: str, max_tokens: int) -> str:
        ...
        if not self.is_supported_model:
            # 对于不支持的模型，使用字符估算截断 [EN] For unsupported models, use character estimate truncation
            ...
            if len(token_ids) <= max_tokens:
                ...
                # 去除多余的特殊前缀（如BOS等） [EN] Remove redundant special prefixes (such as BOS, etc.)
                result = decoded_text.lstrip("<|begin▁of▁sentence|>").lstrip()  # 若需更灵活，可用正则处理 [EN] If you need more flexibility, you can use regular processing

        logger.info(f"文本截断总耗时: {time.time() - start_time:.4f}秒")
        return result
    
    # def truncate_text(self, text: str, max_tokens: int) -> str:
    #     """按照指定的最大token数量截断文本""" [EN] """Truncate text according to the specified maximum number of tokens"""
    #     start_time = time.time()
    #     if max_tokens < 1:
    #         raise ValueError("max_tokens必须大于0") [EN] raise ValueError("max_tokens must be greater than 0")
    ...
    #     logger.info(f"文本截断总耗时: {time.time() - start_time:.4f}秒") [EN] logger.info(f"Total time spent on text truncation: {time.time() - start_time:.4f} seconds")
    #     return result
...
if __name__ == "__main__":
    ...
    # 测试模型支持检查 [EN] Test model support checks
    ...
    print(f"Is current model supported: {deepseek_r1_distill_qwen_32b_tokenizer.is_supported_model}")  # 使用实例属性 [EN] Using instance properties
