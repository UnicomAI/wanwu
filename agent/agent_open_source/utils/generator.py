...
# 创建 RedisClient 实例 [EN] Create a RedisClient instance
...
#     """构造错误响应的函数，并设置状态码""" [EN] """Construct an error response function and set the status code"""
...
def generate_stream_response(query, rewrite_query, prompt, response, qa_type, history,ignore_in_history = False, need_search_list=True,search_list=[],gen_file_url_list=[],upload_file_url='',non_llm_response=None,func_name = '',func_params = '',thought_inference = '',request_begin_time = None,request_id="",session_id="", agent_id="",resourceId = ""):
    ...
    if response:
        ...
        if isinstance(response, requests.models.Response):
            ...
            # logger.info(f"{request_id} {query[:QUERY_SHORT]} ---> 原始大模型llm_response:\n{response.text}" ) [EN] logger.info(f"{request_id} {query[:QUERY_SHORT]} ---> Original large model llm_response:\n{response.text}" ) 
            ...
            try:
                for i, line in enumerate(response.iter_lines(decode_unicode=True)):
                    ...
                    if line.startswith("data:") and not line.startswith('data:""'):
                        ...
                        if not datajson['data']['choices'][0]['finish_reason']:
                            ...
                            # 如果是代码解释器类型 且 代码解释器反馈结果（datajson）中的gen_file_url_list不为空时，新加一段关于生成文件的markdown话术 [EN] If it is a code interpreter type and the gen_file_url_list in the code interpreter feedback result (datajson) is not empty, add a new markdown phrase about the generated file.
                                ...
                                # 在加入对话历史前将引文脚本和思考过程标签清空 [EN] Clear citation script and thought process tags before adding to conversation history                                                              
                                ...
                                if session_id:
                                    ...
                                    #prompt不透传给用户，只存云端 [EN] The prompt is not transmitted transparently to the user and is only stored in the cloud.
                ...
                # 生成返回结果 [EN] Generate return results
                result = {
                    "code": 0,
                    "agent_id": agent_id,
                    "session_id": session_id,
            ...
            # langchain sdk返回结果 [EN] langchain sdk returns results
            try:
                for i,line in enumerate(response):   
                    ...
                    ###区分function_call的输出 [EN] ##Distinguish the output of function_call
                    ...
                    if not line.response_metadata.get("finish_reason","") or line.response_metadata.get("finish_reason", "") == "tool_calls":
                        ...
                        result = {
                            ...
                            "history": [],
                            "finish": finish_reason,
                            "usage": {}, # 流式过程不必赋值 [EN] Streaming process does not require assignment
                            "model": model
                        }
                        ...
                        if not ignore_in_history: 
                            ...
                            # print(datajson['data']['choices'][0]['finish_reason'])
                            # print(gen_file_url_list)    
                            # # 在加入对话历史前将引文脚本和思考过程标签清空 [EN] # Clear the quote script and thought process tags before adding them to the conversation history                                                              
                            subjson["response"] = clean_response_content(complete_content)
                            subjson["gen_file_url_list"] = gen_file_url_list
                            subjson["qa_type"] = qa_type
                            subjson["func_name"] = func_name
    ...
    # LLM模型输出为空 [EN] LLM model output is empty    
...
def generate_non_stream_response(query, rewrite_query, prompt, llm_response, qa_type, history, 
    ...
    # **1. 处理 AIMessage 响应** [EN] **1. Processing AIMessage response**
    ...
    # **2. 处理非 LLM 响应** [EN] **2. Handling non-LLM responses**
    elif non_llm_response or non_llm_response == '':
        if qa_type in [qa_types["ACTION"], qa_types["FUNC_CALL"]]:
            thought_inference = f"{thought_inference}\n{non_llm_response}"
    ...
    # **3. 处理 HTTP 异常** [EN] **3. Handling HTTP exceptions**
    else:
